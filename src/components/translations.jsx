export const translations = {
  en: {
    // Navbar
    nav_events: 'Events',
    nav_tokens: 'Tokens',
    nav_bet: 'Bet',
    nav_social: 'Social',
    nav_explore: 'Explore',
    nav_tools: 'Tools',
    nav_account: 'Account',
    nav_my_profile: 'My Profile',
    nav_watchlist: 'Watchlist',
    nav_notifications: 'Notifications',
    nav_admin: 'Admin Panel',
    nav_navigation: 'Navigation',
    nav_settings: 'Settings',
    nav_language: 'Language',
    nav_analytics: 'Analytics',
    nav_nft_portfolio: 'NFT Portfolio',
    nav_web3: 'Web3 & Wallet',
    
    // HUD
    hud_system: 'System Online',
    hud_ai: 'AI Secretary: Active',
    
    // Hero
    hero_eyebrow: '// Global Street Sports Platform //',
    hero_tagline: 'ENERGY · VIBES · COMMUNITY · LIFE',
    hero_cta_athlete: 'JOIN AS ATHLETE',
    hero_cta_spectator: 'WATCH AS SPECTATOR',
    hero_cta_tokens: 'GET ATHLETE TOKENS',
    hero_scroll: 'SCROLL FOR EVENTS',
    
    // Events
    events_title: 'EVENTS',
    events_subtitle: 'Live & Digital Circuit',
    events_loading: 'LOADING EVENTS...',
    events_empty: 'No events yet. Stay tuned.',
    event_spots: 'spots',
    event_live: 'LIVE NOW',
    event_ended: 'ENDED',
    event_register_athlete: 'Register Athlete',
    event_register_spectator: 'Get Ticket',
    event_watch_stream: 'WATCH LIVE',
    event_watch_vod: 'WATCH REPLAY',
    
    // Registration Flow
    reg_step_info: 'Info',
    reg_step_contract: 'Contract',
    reg_step_signature: 'Signature',
    reg_step_upload: 'Upload',
    reg_step_confirm: 'Confirm',
    reg_athlete_title: 'Athlete Registration',
    reg_spectator_title: 'Spectator Ticket',
    reg_first_name: 'First Name',
    reg_last_name: 'Last Name',
    reg_email: 'Email',
    reg_phone: 'Phone Number',
    reg_dob: 'Date of Birth',
    reg_sport: 'Sport Discipline',
    reg_id_upload: 'Upload ID Document',
    reg_id_hint: 'JPG, PNG or PDF - Max 5MB',
    reg_contract_title: 'Liability Waiver & Code of Conduct',
    reg_contract_accept: 'I have read and accept the terms',
    reg_signature_title: 'Digital Signature',
    reg_signature_hint: 'Sign with your finger or mouse',
    reg_signature_clear: 'Clear',
    reg_back: 'Back',
    reg_next: 'Next',
    reg_submit: 'Submit Registration',
    reg_submitting: 'Processing...',
    reg_close: '✕ CLOSE',
    reg_upload_error: 'Upload failed. Please try again.',
    reg_upload_size: 'File too large. Maximum 5MB allowed.',
    
    // Success Modal
    success_athlete_title: 'ATHLETE REGISTERED',
    success_spectator_title: 'TICKET CONFIRMED',
    success_athlete_msg: 'Your registration is confirmed. Check your email for the QR code and event details.',
    success_spectator_msg: 'Your ticket is ready. Check your email for the QR code to enter the venue.',
    success_ticket: 'YOUR TICKET',
    success_seat: 'Seat',
    success_zone: 'Zone',
    success_close: 'GOT IT',
    
    // Tokens
    tokens_title: 'ATHLETE TOKENS',
    tokens_subtitle: 'Digital Collectibles',
    tokens_intro: 'Support your favorite athletes and unlock exclusive perks. Tokens grant voting rights, betting access, and revenue sharing from athlete sponsorships.',
    tokens_soon: 'Token marketplace launching soon. Follow our social channels for the drop announcement.',
    tokens_cta: 'JOIN WAITLIST',
    
    // Betting
    bet_title: 'ETHICAL BETTING',
    bet_subtitle: 'Prediction Rewards',
    bet_locked_title: 'Token Required',
    bet_locked_msg: 'Purchase an athlete token to unlock prediction challenges and win exclusive prizes.',
    bet_locked_cta: 'Get Tokens',
    bet_my_bets: 'MY PREDICTIONS',
    bet_empty: 'No active bets. Visit event cards and tap 🎯 to predict outcomes.',
    bet_how_title: 'How It Works',
    bet_how_1: '1. Own an athlete token',
    bet_how_2: '2. Predict event outcomes',
    bet_how_3: '3. Win exclusive prizes',
    bet_how_4: '4. Redeem rewards on-site',
    bet_disclaimer: '⚠️ NO MONETARY PRIZES. All rewards are physical merchandise, discount codes, or event perks. This is an ethical gamification system.',
    
    // Social Rewards (for spectators)
    social_rewards_title: 'SPECTATOR PERKS',
    social_rewards_subtitle: 'Follow & Earn',
    social_rewards_msg: 'No token? No problem. Follow us on all platforms and show your phone at the venue for free merch, drinks, and raffle entries.',
    social_rewards_list: '✓ Follow all 5 social accounts\n✓ Show proof at the event booth\n✓ Get instant freebies',
    social_rewards_cta: 'VIEW SOCIAL LINKS',
    
    // Sponsor
    sponsor_title: 'PARTNER WITH US',
    sponsor_subtitle: 'Reach the Next Generation',
    sponsor_value: 'Street Dinamics connects your brand with 13–30 year-old athletes and fans globally. Multi-platform visibility, live event activation, NFT drops, and authentic community engagement across UAE, Europe, and international markets.',
    sponsor_demo: 'Ages 13–30 • 50K+ global reach • Live & digital events • Token integration',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'We respond within 48h. Media kits available on request.',
    
    // Social Modal
    social_modal_title: 'STREET DINAMICS',
    social_modal_subtitle: 'Follow Us Everywhere',
    social_instagram: 'Instagram',
    social_tiktok: 'TikTok',
    social_youtube: 'YouTube',
    social_kick: 'Kick — Live Streams',
    social_snapchat: 'Snapchat',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, Dubai, UAE) — All Rights Reserved',
    
    // Onboarding
    onboard_welcome: 'WELCOME TO STREET DINAMICS',
    onboard_choose: 'Choose Your Path',
    onboard_athlete: 'ATHLETE',
    onboard_athlete_desc: 'Compete in events, build your profile, earn tokens, and grow your fan base',
    onboard_spectator: 'SPECTATOR',
    onboard_spectator_desc: 'Watch events, support athletes, collect tokens, and win prizes',
    onboard_profile_title: 'Complete Your Profile',
    onboard_photo: 'Upload Photo',
    onboard_phone: 'Phone',
    onboard_dob: 'Date of Birth',
    onboard_country: 'Country',
    onboard_city: 'City',
    onboard_sports: 'Sports (comma-separated)',
    onboard_sports_placeholder: 'e.g. Parkour, BMX, Skateboarding',
    onboard_bio: 'Bio',
    onboard_bio_placeholder: 'Tell us about yourself and your journey...',
    onboard_fav_sports: 'Favorite Sports (comma-separated)',
    onboard_back: 'Back',
    onboard_complete: 'Complete Setup',
    onboard_creating: 'Creating Profile...',
    
    // Admin
    admin_panel: 'ADMIN PANEL',
    admin_create_event: 'CREATE EVENT',
    admin_go_live: 'GO LIVE',
    admin_end_event: 'END EVENT',
    admin_kick_url: 'Kick Live URL',
    admin_youtube_url: 'YouTube Live URL',
    admin_save_links: 'Save Live Links',
    admin_change_pass: 'Change Password',
    admin_new_pass: 'New Password',
    admin_confirm_pass: 'Confirm Password',
    
    // Token Marketplace
    marketplace_title: 'TOKEN MARKETPLACE',
    marketplace_subtitle: 'Digital Collectibles',
    marketplace_total_athletes: 'Total Athletes',
    marketplace_available: 'Available Tokens',
    marketplace_floor: 'Floor Price',
    marketplace_loading: 'LOADING TOKENS...',
    marketplace_no_match: 'No tokens match your filters',
    
    // Attendance Type
    attendance_title: 'CHOOSE ATTENDANCE TYPE',
    attendance_in_person: 'IN PERSON',
    attendance_in_person_desc: 'Attend the event in person at {location}. Get physical ticket with seat assignment and venue access.',
    attendance_online: 'ONLINE',
    attendance_online_desc: 'Watch the live stream remotely. Receive digital pass and access to exclusive online features.',
    attendance_select: 'SELECT',

    // Sponsor Section
    sponsor_demographics: 'High-Value Youth Demographics',
    sponsor_platform: 'Multiplatform Presence',
    sponsor_worldwide: 'Worldwide Presence',
    sponsor_visibility_title: 'MULTI-PLATFORM VISIBILITY',
    sponsor_visibility_desc: 'Brand exposure across live events, digital streams, and social media channels',
    sponsor_engagement_title: 'DIRECT YOUTH ENGAGEMENT',
    sponsor_engagement_desc: 'Authentic connection with 13-30 year-old athletes and fans who drive culture',
    sponsor_global_title: 'GLOBAL REACH',
    sponsor_global_desc: 'Presence in UAE, Europe, and expanding international markets with high engagement',
    sponsor_nft_title: 'NFT & TOKEN INTEGRATION',
    sponsor_nft_desc: 'Innovative digital assets that keep your brand at the forefront of Web3 culture',

    // NFT Marketplace
    nft_subtitle: 'Historic Moments',
    nft_title: 'NFT COLLECTION CARDS',
    nft_description: 'Each NFT captures a unique historic moment. Limited supply. Value increases with athlete performance.',
    nft_filters: 'Advanced Filters',
    nft_filters_title: 'ADVANCED FILTERS',
    nft_filters_tier: 'Card Tier',
    nft_filters_tier_all: 'All Tiers',
    nft_filters_tier_rising: 'Rising Stars',
    nft_filters_tier_elite: 'Elite Performers',
    nft_filters_tier_champions: 'Champions',
    nft_filters_tier_hall: 'Hall of Fame',
    nft_filters_athlete: 'Athlete',
    nft_filters_athlete_search: 'Search athlete...',
    nft_filters_athlete_notfound: 'No athletes found',
    nft_filters_price: 'Price Range (€)',
    nft_filters_price_min: 'Min',
    nft_filters_price_max: 'Max',
    nft_filters_availability: 'Availability',
    nft_filters_availability_all: 'All Status',
    nft_filters_availability_stock: 'In Stock',
    nft_filters_availability_limited: 'Limited Stock',
    nft_filters_availability_sold: 'Sold Out',
    nft_filters_availability_coming: 'Coming Soon',
    nft_filters_clear: 'Clear All Filters',

    // Watchlist
    watchlist_title: 'MY WATCHLIST',
    watchlist_description: 'Track price changes and new listings for your favorite assets',
    watchlist_empty: 'No watchlist items yet. Add athletes or NFT drops to track them here.',
    watchlist_watch: 'Watch',
    watchlist_watched: 'Watched',
    watchlist_price_alerts: 'Price Alerts On',
    watchlist_price_alerts_off: 'Price Alerts Off',
    watchlist_listings: 'Listings On',
    watchlist_listings_off: 'Listings Off',
    watchlist_remove: 'Remove',

    // Secondary Market
    secondary_subtitle: 'Peer-to-Peer Trading',
    secondary_title: 'SECONDARY MARKET',
    secondary_description: 'Buy and sell athlete tokens & NFTs directly from other collectors. 5% platform fee on all trades.',
    secondary_list_for_sale: 'List for Sale',
    secondary_no_listings: 'No listings available',
    secondary_quantity: 'Quantity',
    secondary_per_unit: 'per unit',
    secondary_total: 'Total',
    secondary_processing: 'Processing...',
    secondary_buy_now: 'Buy Now',
    secondary_create_listing: 'Create Listing',
    secondary_select_asset: 'Select Asset to Sell',
    secondary_listing_price: 'Listing Price (EUR)',
    secondary_enter_price: 'Enter price per unit',
    secondary_sale_price: 'Sale Price',
    secondary_platform_fee: 'Platform Fee (5%)',
    secondary_you_receive: 'You Receive',
    secondary_list_success: 'Listed on marketplace!',
    secondary_list_error: 'Failed to create listing',
    secondary_purchase_success: 'Purchase successful!',
    secondary_purchase_error: 'Purchase failed',
    secondary_fill_fields: 'Please fill in all fields',
    secondary_cancel: 'Cancel',
    secondary_creating: 'Creating...',
    secondary_create: 'Create Listing',
    secondary_top_assets: 'TOP ASSETS',
    secondary_recent_transactions: 'RECENT TRANSACTIONS',
    secondary_activity_empty: 'No activity yet',
    secondary_transactions_empty: 'No transactions yet',

    // Gamification
    gamification_subtitle: 'Fan Engagement Hub',

    // Global Leaderboard
    leaderboard_subtitle: 'Global Ranking System',
    leaderboard_title: 'CHAMPIONS BOARD',
    leaderboard_showing: 'Showing',
    leaderboard_empty: 'No athletes on the leaderboard yet',
    leaderboard_loading: 'Loading rankings...',

    // Subscriptions
    my_subscriptions: 'My Subscriptions',

    // Analytics page
    analytics_admin: 'ADMIN ANALYTICS',
    analytics_personal: 'PERSONAL ANALYTICS',
    analytics_title: 'PERFORMANCE METRICS',
    analytics_admin_desc: 'Monitor platform-wide engagement, voting trends, and betting performance',
    analytics_personal_desc: 'Track your engagement, voting accuracy, and betting performance over time',
    analytics_engagement: 'Engagement',
    analytics_voting: 'Voting',
    analytics_betting: 'Betting',

    // Voting Hub
    voting_hub_title: 'Voting Hub',
    voting_no_live: 'No live events available',
    voting_view: 'View Votes →',
    voting_back: '← Back to Events',
    voting_campaigns: 'Voting Campaigns',
    voting_leaderboard: 'Leaderboard',
    voting_manage: 'Manage Campaigns',
    voting_no_campaigns: 'No active voting campaigns',

    // NFT Dashboard
    nft_dashboard_portfolio: '// PORTFOLIO //',
    nft_dashboard_title: 'NFT COLLECTION',
    nft_dashboard_total_value: 'Total Value',
    nft_dashboard_nfts: 'NFTs',
    nft_dashboard_invested: 'Total Invested',
    nft_dashboard_purchase: 'Purchase price',
    nft_dashboard_profit: 'Profit / Loss',
    nft_dashboard_transactions: 'Transactions',
    nft_dashboard_buy_sell: 'Buy & Sell',
    nft_dashboard_my_nfts: 'MY NFTS',
    nft_dashboard_sort_value: 'Sort by Value',
    nft_dashboard_sort_profit: 'Sort by Profit',
    nft_dashboard_sort_recent: 'Sort by Recent',
    nft_dashboard_empty: "You haven't owned any NFTs yet",
    nft_dashboard_purchase_label: 'PURCHASE',
    nft_dashboard_current: 'CURRENT',
    nft_dashboard_pl: 'P/L',
    nft_dashboard_minted: 'MINTED',
    nft_dashboard_return: 'Return',
    nft_dashboard_last_trade: 'Last Trade',
    nft_dashboard_history: 'TRANSACTION HISTORY',
    nft_dashboard_card: 'CARD',
    nft_dashboard_type: 'TYPE',
    nft_dashboard_price: 'PRICE',
    nft_dashboard_date: 'DATE',

    // Athlete Profile
    athlete_edit_profile: 'EDIT PROFILE',
    athlete_events_stat: 'Events',
    athlete_wins_stat: 'Wins',
    athlete_podiums_stat: 'Podiums',
    athlete_rating_stat: 'Rating',
    athlete_universal_score: 'UNIVERSAL SCORE',
    athlete_badges: 'BADGES',
    athlete_royalty: 'ROYALTY EARNINGS',
    athlete_event_history: 'EVENT HISTORY',
    athlete_no_events: 'NO EVENTS YET',
    athlete_achievements: 'ACHIEVEMENTS',
    athlete_not_found: 'ATHLETE NOT FOUND',
    athlete_invalid_link: 'Invalid profile link',

    // Web3 Page
    web3_hub: '// Web3 Hub //',
    web3_title: 'WEB3 & WALLET',
    web3_connectivity: 'Connectivity',
    web3_connect_wallet: 'CONNECT WALLET',
    web3_connect_desc: 'Connect your Web3 wallet to unlock NFT minting, token staking, and on-chain rewards.',
    web3_staking: 'Staking',
    web3_token_staking: 'TOKEN STAKING',

    // User Profile Dashboard
    upd_collection_value: 'Collection Value',
    upd_nfts_owned: 'NFTs owned',
    upd_rewards_earned: 'Rewards Earned',
    upd_total_rewards: 'total rewards',
    upd_web3_wallets: 'Web3 Wallets',
    upd_connected_wallets: 'Connected wallets',
    upd_verified_athlete: '✓ Verified Athlete',
    upd_nft_collection: 'Your NFT Collection',
    upd_reward_history: 'Reward History',
    upd_redeemed: '✓ Redeemed',
    upd_pending: 'Pending',
    upd_more_rewards: 'more rewards',
    upd_wallet_management: 'Web3 Wallet Management',
    upd_add_wallet: '+ Add Wallet',
    upd_no_wallets: 'No wallets connected yet',
    upd_no_wallets_desc: 'Link your Ethereum/Polygon wallet for Web3 transactions',
    upd_remove: 'Remove',
    upd_wallet_updated: 'Wallets updated',
    upd_wallet_update_fail: 'Failed to update wallets',
    upd_wallet_exists: 'Wallet already linked',
    upd_wallet_prompt: 'Enter wallet address (Ethereum/Polygon):',
    upd_remove_confirm: 'Remove this wallet?',

    // Fan Status Dashboard
    fsd_fan_progression: 'FAN PROGRESSION',
    fsd_your_status: 'YOUR STATUS',
    fsd_subtitle: 'Earn tokens, collect NFTs, and unlock exclusive perks as you progress through the tiers',
    fsd_earnings_multiplier: 'EARNINGS MULTIPLIER',
    fsd_tokens_spent: 'Tokens Spent',
    fsd_rarity_score: 'Rarity Score',
    fsd_nfts_owned: 'NFTs Owned',
    fsd_rewards: 'Rewards',
    fsd_progress_to: 'Progress to',
    fsd_tokens_until: 'tokens until',
    fsd_active_perks: 'Active Perks',
    fsd_all_tiers: 'All Tiers',
    fsd_current: 'Current',
    fsd_requirement: 'REQUIREMENT',
    fsd_starting_tier: 'Starting tier',
    fsd_tokens_spent_req: 'tokens spent',
    fsd_perks: 'PERKS',
    fsd_more: 'more...',
    fsd_multiplier: 'multiplier',
    fsd_level: 'Level',
    fsd_of: 'of',
    fsd_login_prompt: 'Login to view your fan status',

    // Leaderboard
    lb_sport: 'Sport',
    lb_all_sports: 'All Sports',
    lb_sort: 'Sort',
    lb_top_score: 'Top Score',
    lb_trending_up: 'Trending Up',
    lb_trending: 'Trending',
    lb_tech: 'Tech',
    lb_engagement: 'Engagement',
    lb_rank: 'Rank',
    lb_athlete: 'Athlete',
    lb_score: 'Score',

    // Enhanced User Profile
    enhanced_loading: 'Loading profile...',
    enhanced_ugc_label: 'UGC Submissions',
    enhanced_engagement_label: 'Total Engagement',
    enhanced_points_label: 'Points Earned',
    enhanced_tokens_label: 'Tokens Owned',
    enhanced_tab_overview: 'overview',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'badges',
    enhanced_tab_events: 'events',
    enhanced_tab_betting: 'betting',
    enhanced_ugc_title: 'UGC SUBMISSION HISTORY',
    enhanced_no_ugc: 'No UGC submissions yet',
    enhanced_badges_title: 'ACHIEVEMENTS & BADGES',
    enhanced_events_title: 'SAVED EVENTS & INTERESTS',
    enhanced_no_events: 'No saved events yet',
    enhanced_creator_level: 'Content Creator Level',
    enhanced_engagement_score: 'Engagement Score',
    enhanced_rewards_unlocked: 'Rewards Unlocked',
  },
  
  it: {
    // Navbar
    nav_events: 'Eventi',
    nav_tokens: '🎫 Token',
    nav_bet: '🎯 Scommetti',
    nav_social: '🔗 Social',
    nav_explore: 'Esplora',
    nav_tools: 'Strumenti',
    nav_account: 'Account',
    nav_my_profile: 'Il Mio Profilo',
    nav_watchlist: 'Lista di Controllo',
    nav_notifications: 'Notifiche',
    nav_admin: 'Pannello Admin',
    nav_navigation: 'Navigazione',
    nav_settings: 'Impostazioni',
    nav_language: 'Lingua',
    nav_analytics: 'Analisi',
    nav_nft_portfolio: 'Portfolio NFT',
    nav_web3: 'Web3 & Wallet',
    
    // HUD
    hud_system: 'Sistema Attivo',
    hud_ai: 'Segretaria AI: Attiva',
    
    // Hero
    hero_eyebrow: '// Piattaforma Sportiva Globale //',
    hero_tagline: 'Tornei · Scouting · Comunità · 13–30',
    hero_cta_athlete: 'ISCRIVITI COME ATLETA',
    hero_cta_spectator: 'GUARDA COME SPETTATORE',
    hero_cta_tokens: '🎫 OTTIENI TOKEN ATLETA',
    hero_scroll: '↓ SCORRI PER EVENTI',
    
    // Events
    events_title: 'EVENTI',
    events_subtitle: 'Circuito Live & Digitale',
    events_loading: 'CARICAMENTO EVENTI...',
    events_empty: 'Nessun evento per ora. Resta sintonizzato.',
    event_spots: 'posti',
    event_live: 'LIVE ORA',
    event_ended: 'CONCLUSO',
    event_register_athlete: '🏅 Iscriviti Atleta',
    event_register_spectator: '🎫 Ottieni Biglietto',
    event_watch_stream: '▶️ GUARDA LIVE',
    event_watch_vod: '📺 GUARDA REPLAY',
    
    // Registration Flow
    reg_step_info: 'Info',
    reg_step_contract: 'Contratto',
    reg_step_signature: 'Firma',
    reg_step_upload: 'Carica',
    reg_step_confirm: 'Conferma',
    reg_athlete_title: 'Registrazione Atleta',
    reg_spectator_title: 'Biglietto Spettatore',
    reg_first_name: 'Nome',
    reg_last_name: 'Cognome',
    reg_email: 'Email',
    reg_phone: 'Telefono',
    reg_dob: 'Data di Nascita',
    reg_sport: 'Disciplina Sportiva',
    reg_id_upload: 'Carica Documento',
    reg_id_hint: 'JPG, PNG o PDF - Max 5MB',
    reg_contract_title: 'Liberatoria & Codice di Condotta',
    reg_contract_accept: 'Ho letto e accetto i termini',
    reg_signature_title: 'Firma Digitale',
    reg_signature_hint: 'Firma con il dito o il mouse',
    reg_signature_clear: 'Cancella',
    reg_back: 'Indietro',
    reg_next: 'Avanti',
    reg_submit: 'Invia Registrazione',
    reg_submitting: 'Elaborazione...',
    reg_close: '✕ CHIUDI',
    reg_upload_error: 'Caricamento fallito. Riprova.',
    reg_upload_size: 'File troppo grande. Massimo 5MB.',
    
    // Success Modal
    success_athlete_title: 'ATLETA REGISTRATO',
    success_spectator_title: 'BIGLIETTO CONFERMATO',
    success_athlete_msg: 'La tua registrazione è confermata. Controlla la tua email per il QR code e i dettagli dell\'evento.',
    success_spectator_msg: 'Il tuo biglietto è pronto. Controlla la tua email per il QR code per entrare nella sede.',
    success_ticket: 'IL TUO BIGLIETTO',
    success_seat: 'Posto',
    success_zone: 'Zona',
    success_close: 'HO CAPITO',
    
    // Tokens
    tokens_title: 'TOKEN ATLETA',
    tokens_subtitle: 'Oggetti Digitali',
    tokens_intro: 'Supporta i tuoi atleti preferiti e sblocca vantaggi esclusivi. I token garantiscono diritti di voto, accesso alle scommesse e condivisione dei ricavi dalle sponsorizzazioni degli atleti.',
    tokens_soon: 'Il marketplace dei token sarà lanciato presto. Segui i nostri canali social per l\'annuncio.',
    tokens_cta: 'ISCRIVITI ALLA LISTA',
    
    // Betting
    bet_title: 'SCOMMESSE ETICHE',
    bet_subtitle: 'Premi per Previsioni',
    bet_locked_title: 'Token Richiesto',
    bet_locked_msg: 'Acquista un token atleta per sbloccare le sfide di previsione e vincere premi esclusivi.',
    bet_locked_cta: 'Ottieni Token',
    bet_my_bets: 'LE MIE PREVISIONI',
    bet_empty: 'Nessuna scommessa attiva. Visita le card degli eventi e tocca 🎯 per prevedere i risultati.',
    bet_how_title: 'Come Funziona',
    bet_how_1: '1. Possiedi un token atleta',
    bet_how_2: '2. Prevedi i risultati degli eventi',
    bet_how_3: '3. Vinci premi esclusivi',
    bet_how_4: '4. Riscatta i premi sul posto',
    bet_disclaimer: '⚠️ NESSUN PREMIO IN DENARO. Tutti i premi sono merchandising fisico, codici sconto o vantaggi per eventi. Questo è un sistema di gamificazione etico.',
    
    // Social Rewards
    social_rewards_title: 'VANTAGGI SPETTATORE',
    social_rewards_subtitle: 'Segui & Vinci',
    social_rewards_msg: 'Nessun token? Nessun problema. Seguici su tutte le piattaforme e mostra il tuo telefono alla sede per merch gratuito, bevande e biglietti della lotteria.',
    social_rewards_list: '✓ Segui tutti i 5 account social\n✓ Mostra la prova allo stand dell\'evento\n✓ Ottieni omaggi istantanei',
    social_rewards_cta: 'VEDI LINK SOCIAL',
    
    // Sponsor
    sponsor_title: 'COLLABORA CON NOI',
    sponsor_subtitle: 'Raggiungi la Nuova Generazione',
    sponsor_value: 'Street Dinamics collega il tuo brand con atleti e fan di 13–30 anni a livello globale. Visibilità multi-piattaforma, eventi dal vivo, NFT drop e coinvolgimento autentico in UAE, Europa e mercati internazionali.',
    sponsor_demo: 'Età 13–30 • 50K+ reach globale • Eventi live & digitali • Integrazione token',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'Rispondiamo entro 48h. Kit media disponibili su richiesta.',
    
    // Social Modal
    social_modal_title: 'STREET DINAMICS',
    social_modal_subtitle: 'Seguici Ovunque',
    social_instagram: 'Instagram',
    social_tiktok: 'TikTok',
    social_youtube: 'YouTube',
    social_kick: 'Kick — Live Stream',
    social_snapchat: 'Snapchat',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, Dubai, UAE) — Tutti i Diritti Riservati',
    
    // Onboarding
    onboard_welcome: 'BENVENUTO A STREET DINAMICS',
    onboard_choose: 'Scegli il Tuo Percorso',
    onboard_athlete: 'ATLETA',
    onboard_athlete_desc: 'Gareggia negli eventi, costruisci il tuo profilo, guadagna token e fai crescere la tua fanbase',
    onboard_spectator: 'SPETTATORE',
    onboard_spectator_desc: 'Guarda gli eventi, supporta gli atleti, colleziona token e vinci premi',
    onboard_profile_title: 'Completa il Tuo Profilo',
    onboard_photo: 'Carica Foto',
    onboard_phone: 'Telefono',
    onboard_dob: 'Data di Nascita',
    onboard_country: 'Paese',
    onboard_city: 'Città',
    onboard_sports: 'Sport (separati da virgola)',
    onboard_sports_placeholder: 'es. Parkour, BMX, Skateboarding',
    onboard_bio: 'Bio',
    onboard_bio_placeholder: 'Raccontaci di te e del tuo percorso...',
    onboard_fav_sports: 'Sport Preferiti (separati da virgola)',
    onboard_back: 'Indietro',
    onboard_complete: 'Completa Configurazione',
    onboard_creating: 'Creazione Profilo...',
    
    // Admin
    admin_panel: 'PANNELLO ADMIN',
    admin_create_event: '+ CREA EVENTO',
    admin_go_live: 'VAI LIVE',
    admin_end_event: 'TERMINA EVENTO',
    admin_kick_url: 'URL Live Kick',
    admin_youtube_url: 'URL Live YouTube',
    admin_save_links: 'Salva Link Live',
    admin_change_pass: 'Cambia Password',
    admin_new_pass: 'Nuova Password',
    admin_confirm_pass: 'Conferma Password',
    
    // Token Marketplace
    marketplace_title: 'MARKETPLACE TOKEN',
    marketplace_subtitle: 'Oggetti Digitali',
    marketplace_total_athletes: 'Atleti Totali',
    marketplace_available: 'Token Disponibili',
    marketplace_floor: 'Prezzo Base',
    marketplace_loading: 'CARICAMENTO TOKEN...',
    marketplace_no_match: 'Nessun token corrisponde ai filtri',
    
    // Attendance Type
    attendance_title: 'SCEGLI TIPO DI PARTECIPAZIONE',
    attendance_in_person: 'DAL VIVO',
    attendance_in_person_desc: 'Partecipa di persona all\'evento a {location}. Ricevi biglietto fisico con posto assegnato e accesso alla sede.',
    attendance_online: 'ONLINE',
    attendance_online_desc: 'Guarda la diretta streaming da remoto. Ricevi pass digitale e accesso a funzionalità online esclusive.',
    attendance_select: 'SELEZIONA',

    // Secondary Market
    secondary_subtitle: 'Commercio da Pari a Pari',
    secondary_title: 'MERCATO SECONDARIO',
    secondary_description: 'Compra e vendi token atleti e NFT direttamente da altri collezionisti. 5% di commissione piattaforma su tutti gli scambi.',
    secondary_list_for_sale: 'Metti in Vendita',
    secondary_no_listings: 'Nessuna lista disponibile',
    secondary_quantity: 'Quantità',
    secondary_per_unit: 'per unità',
    secondary_total: 'Totale',
    secondary_processing: 'Elaborazione...',
    secondary_buy_now: 'Acquista Ora',
    secondary_create_listing: 'Crea una Lista',
    secondary_select_asset: 'Seleziona Asset da Vendere',
    secondary_listing_price: 'Prezzo di Lista (EUR)',
    secondary_enter_price: 'Inserisci il prezzo per unità',
    secondary_sale_price: 'Prezzo di Vendita',
    secondary_platform_fee: 'Commissione Piattaforma (5%)',
    secondary_you_receive: 'Riceverai',
    secondary_list_success: 'Elencato sul marketplace!',
    secondary_list_error: 'Impossibile creare la lista',
    secondary_purchase_success: 'Acquisto riuscito!',
    secondary_purchase_error: 'Acquisto non riuscito',
    secondary_fill_fields: 'Compila tutti i campi',
    secondary_cancel: 'Annulla',
    secondary_creating: 'Creazione in corso...',
    secondary_create: 'Crea una Lista',
    secondary_top_assets: 'ASSET PRINCIPALI',
    secondary_recent_transactions: 'TRANSAZIONI RECENTI',
    secondary_activity_empty: 'Nessuna attività ancora',
    secondary_transactions_empty: 'Nessuna transazione ancora',

    // Gamification
    gamification_subtitle: 'Hub di Coinvolgimento Fan',

    // Global Leaderboard
    leaderboard_subtitle: 'Sistema di Ranking Globale',
    leaderboard_title: 'BOARD DEI CAMPIONI',
    leaderboard_showing: 'Mostra',
    leaderboard_empty: 'Nessun atleta sulla classifica ancora',
    leaderboard_loading: 'Caricamento ranking...',

    // Subscriptions
    my_subscriptions: 'Le Mie Iscrizioni',

    // Analytics page
    analytics_admin: 'ANALYTICS ADMIN',
    analytics_personal: 'ANALYTICS PERSONALI',
    analytics_title: 'METRICHE DI PERFORMANCE',
    analytics_admin_desc: 'Monitora il coinvolgimento della piattaforma, le tendenze di voto e le performance delle scommesse',
    analytics_personal_desc: 'Traccia il tuo coinvolgimento, la precisione delle votazioni e le performance delle scommesse nel tempo',
    analytics_engagement: 'Coinvolgimento',
    analytics_voting: 'Votazioni',
    analytics_betting: 'Scommesse',

    // Voting Hub
    voting_hub_title: 'Hub Votazioni',
    voting_no_live: 'Nessun evento live disponibile',
    voting_view: 'Vedi Voti →',
    voting_back: '← Torna agli Eventi',
    voting_campaigns: 'Campagne di Voto',
    voting_leaderboard: 'Classifica',
    voting_manage: 'Gestisci Campagne',
    voting_no_campaigns: 'Nessuna campagna di voto attiva',

    // NFT Dashboard
    nft_dashboard_portfolio: '// PORTAFOGLIO //',
    nft_dashboard_title: 'COLLEZIONE NFT',
    nft_dashboard_total_value: 'Valore Totale',
    nft_dashboard_nfts: 'NFT',
    nft_dashboard_invested: 'Totale Investito',
    nft_dashboard_purchase: 'Prezzo acquisto',
    nft_dashboard_profit: 'Profitto / Perdita',
    nft_dashboard_transactions: 'Transazioni',
    nft_dashboard_buy_sell: 'Acquisto & Vendita',
    nft_dashboard_my_nfts: 'I MIEI NFT',
    nft_dashboard_sort_value: 'Ordina per Valore',
    nft_dashboard_sort_profit: 'Ordina per Profitto',
    nft_dashboard_sort_recent: 'Ordina per Recenti',
    nft_dashboard_empty: 'Non possiedi ancora NFT',
    nft_dashboard_purchase_label: 'ACQUISTO',
    nft_dashboard_current: 'ATTUALE',
    nft_dashboard_pl: 'P/L',
    nft_dashboard_minted: 'CONIATO',
    nft_dashboard_return: 'Rendimento',
    nft_dashboard_last_trade: 'Ultimo Scambio',
    nft_dashboard_history: 'STORICO TRANSAZIONI',
    nft_dashboard_card: 'CARTA',
    nft_dashboard_type: 'TIPO',
    nft_dashboard_price: 'PREZZO',
    nft_dashboard_date: 'DATA',

    // Athlete Profile
    athlete_edit_profile: 'MODIFICA PROFILO',
    athlete_events_stat: 'Eventi',
    athlete_wins_stat: 'Vittorie',
    athlete_podiums_stat: 'Podi',
    athlete_rating_stat: 'Valutazione',
    athlete_universal_score: 'PUNTEGGIO UNIVERSALE',
    athlete_badges: 'BADGE',
    athlete_royalty: 'GUADAGNI ROYALTY',
    athlete_event_history: 'STORICO EVENTI',
    athlete_no_events: 'NESSUN EVENTO ANCORA',
    athlete_achievements: 'OBIETTIVI',
    athlete_not_found: 'ATLETA NON TROVATO',
    athlete_invalid_link: 'Link profilo non valido',

    // Enhanced User Profile
    enhanced_loading: 'Caricamento profilo...',
    enhanced_ugc_label: 'Contenuti UGC',
    enhanced_engagement_label: 'Coinvolgimento Totale',
    enhanced_points_label: 'Punti Guadagnati',
    enhanced_tokens_label: 'Token Posseduti',
    enhanced_tab_overview: 'panoramica',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'badge',
    enhanced_tab_events: 'eventi',
    enhanced_tab_betting: 'scommesse',
    enhanced_ugc_title: 'STORICO CONTENUTI UGC',
    enhanced_no_ugc: 'Nessun contenuto UGC ancora',
    enhanced_badges_title: 'OBIETTIVI & BADGE',
    enhanced_events_title: 'EVENTI SALVATI & INTERESSI',
    enhanced_no_events: 'Nessun evento salvato ancora',
    enhanced_creator_level: 'Livello Creatore',
    enhanced_engagement_score: 'Punteggio Coinvolgimento',
    enhanced_rewards_unlocked: 'Premi Sbloccati',
    },
  
  es: {
    // Navbar
    nav_events: 'Eventos',
    nav_tokens: '🎫 Tokens',
    nav_bet: '🎯 Apostar',
    nav_social: '🔗 Social',
    nav_explore: 'Explorar',
    nav_tools: 'Herramientas',
    nav_account: 'Cuenta',
    nav_my_profile: 'Mi Perfil',
    nav_watchlist: 'Lista de Seguimiento',
    nav_notifications: 'Notificaciones',
    nav_admin: 'Panel Admin',
    nav_navigation: 'Navegación',
    nav_settings: 'Configuración',
    nav_language: 'Idioma',
    nav_analytics: 'Analíticas',
    nav_nft_portfolio: 'Portfolio NFT',
    nav_web3: 'Web3 & Wallet',
    
    // HUD
    hud_system: 'Sistema en Línea',
    hud_ai: 'Secretaria IA: Activa',
    
    // Hero
    hero_eyebrow: '// Plataforma Deportiva Global //',
    hero_tagline: 'Torneos · Scouting · Comunidad · 13–30',
    hero_cta_athlete: 'ÚNETE COMO ATLETA',
    hero_cta_spectator: 'VER COMO ESPECTADOR',
    hero_cta_tokens: '🎫 OBTENER TOKENS',
    hero_scroll: '↓ DESPLÁZATE PARA EVENTOS',
    
    // Events
    events_title: 'EVENTOS',
    events_subtitle: 'Circuito en Vivo y Digital',
    events_loading: 'CARGANDO EVENTOS...',
    events_empty: 'No hay eventos aún. Mantente atento.',
    event_spots: 'plazas',
    event_live: 'EN VIVO',
    event_ended: 'FINALIZADO',
    event_register_athlete: '🏅 Registrar Atleta',
    event_register_spectator: '🎫 Obtener Entrada',
    event_watch_stream: '▶️ VER EN VIVO',
    event_watch_vod: '📺 VER REPETICIÓN',
    
    // Registration Flow
    reg_step_info: 'Info',
    reg_step_contract: 'Contrato',
    reg_step_signature: 'Firma',
    reg_athlete_title: 'Registro de Atleta',
    reg_spectator_title: 'Entrada de Espectador',
    reg_first_name: 'Nombre',
    reg_last_name: 'Apellido',
    reg_email: 'Correo',
    reg_phone: 'Teléfono',
    reg_dob: 'Fecha de Nacimiento',
    reg_sport: 'Disciplina Deportiva',
    reg_id_upload: 'Subir Documento',
    reg_id_hint: 'JPG, PNG o PDF - Máx 5MB',
    reg_contract_title: 'Exención de Responsabilidad',
    reg_contract_accept: 'He leído y acepto los términos',
    reg_signature_title: 'Firma Digital',
    reg_signature_hint: 'Firma con tu dedo o ratón',
    reg_signature_clear: 'Borrar',
    reg_back: 'Atrás',
    reg_next: 'Siguiente',
    reg_submit: 'Enviar Registro',
    reg_submitting: 'Procesando...',
    reg_close: '✕ CERRAR',
    reg_upload_error: 'Error al cargar. Inténtalo de nuevo.',
    reg_upload_size: 'Archivo demasiado grande. Máximo 5MB.',
    
    // Success Modal
    success_athlete_title: 'ATLETA REGISTRADO',
    success_spectator_title: 'ENTRADA CONFIRMADA',
    success_athlete_msg: 'Tu registro está confirmado. Revisa tu correo para el código QR.',
    success_spectator_msg: 'Tu entrada está lista. Revisa tu correo para el código QR.',
    success_ticket: 'TU ENTRADA',
    success_seat: 'Asiento',
    success_zone: 'Zona',
    success_close: 'ENTENDIDO',
    
    // Tokens
    tokens_title: 'TOKENS DE ATLETA',
    tokens_subtitle: 'Coleccionables Digitales',
    tokens_intro: 'Apoya a tus atletas favoritos y desbloquea beneficios exclusivos.',
    tokens_soon: 'Marketplace de tokens próximamente.',
    tokens_cta: 'UNIRSE A LISTA',
    
    // Betting
    bet_title: 'APUESTAS ÉTICAS',
    bet_subtitle: 'Recompensas por Predicción',
    bet_locked_title: 'Token Requerido',
    bet_locked_msg: 'Compra un token de atleta para desbloquear desafíos.',
    bet_locked_cta: 'Obtener Tokens',
    bet_my_bets: 'MIS PREDICCIONES',
    bet_empty: 'No hay apuestas activas.',
    bet_how_title: 'Cómo Funciona',
    bet_how_1: '1. Posee un token de atleta',
    bet_how_2: '2. Predice resultados',
    bet_how_3: '3. Gana premios exclusivos',
    bet_how_4: '4. Canjea recompensas',
    bet_disclaimer: '⚠️ SIN PREMIOS MONETARIOS. Sistema de gamificación ético.',
    
    // Social Rewards
    social_rewards_title: 'BENEFICIOS ESPECTADOR',
    social_rewards_subtitle: 'Sigue y Gana',
    social_rewards_msg: '¿Sin token? Síguenos en todas las plataformas para regalos.',
    social_rewards_list: '✓ Sigue las 5 cuentas\n✓ Muestra prueba en el evento\n✓ Obtén regalos',
    social_rewards_cta: 'VER ENLACES SOCIALES',
    
    // Sponsor
    sponsor_title: 'ASÓCIATE CON NOSOTROS',
    sponsor_subtitle: 'Alcanza la Nueva Generación',
    sponsor_value: 'Street Dinamics conecta tu marca con atletas y fans de 13–30 años a nivel global. Visibilidad multi-plataforma, eventos en vivo, NFT drops y compromiso auténtico en EAU, Europa y mercados internacionales.',
    sponsor_demo: 'Edades 13–30 • 50K+ alcance global • Eventos en vivo y digitales • Integración de tokens',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'Respondemos en 48h. Kits de medios disponibles bajo solicitud.',
    
    // Social Modal
    social_modal_title: 'STREET DINAMICS',
    social_modal_subtitle: 'Síguenos en Todas Partes',
    social_instagram: 'Instagram',
    social_tiktok: 'TikTok',
    social_youtube: 'YouTube',
    social_kick: 'Kick — Transmisiones',
    social_snapchat: 'Snapchat',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, Dubai, UAE) — Todos los Derechos Reservados',
    
    // Onboarding
    onboard_welcome: 'BIENVENIDO A STREET DINAMICS',
    onboard_choose: 'Elige Tu Camino',
    onboard_athlete: 'ATLETA',
    onboard_athlete_desc: 'Compite en eventos, construye tu perfil, gana tokens y aumenta tu base de fans',
    onboard_spectator: 'ESPECTADOR',
    onboard_spectator_desc: 'Ve eventos, apoya a atletas, colecciona tokens y gana premios',
    onboard_profile_title: 'Completa Tu Perfil',
    onboard_photo: 'Subir Foto',
    onboard_phone: 'Teléfono',
    onboard_dob: 'Fecha de Nacimiento',
    onboard_country: 'País',
    onboard_city: 'Ciudad',
    onboard_sports: 'Deportes (separados por coma)',
    onboard_sports_placeholder: 'ej. Parkour, BMX, Skateboarding',
    onboard_bio: 'Biografía',
    onboard_bio_placeholder: 'Cuéntanos sobre ti y tu trayectoria...',
    onboard_fav_sports: 'Deportes Favoritos (separados por coma)',
    onboard_back: 'Atrás',
    onboard_complete: 'Completar Configuración',
    onboard_creating: 'Creando Perfil...',
    
    // Admin
    admin_panel: 'PANEL DE ADMIN',
    admin_create_event: '+ CREAR EVENTO',
    admin_go_live: 'IR EN VIVO',
    admin_end_event: 'FINALIZAR EVENTO',
    admin_kick_url: 'URL Live Kick',
    admin_youtube_url: 'URL Live YouTube',
    admin_save_links: 'Guardar Enlaces Live',
    admin_change_pass: 'Cambiar Contraseña',
    admin_new_pass: 'Nueva Contraseña',
    admin_confirm_pass: 'Confirmar Contraseña',
    
    // Token Marketplace
    marketplace_title: 'MARKETPLACE DE TOKENS',
    marketplace_subtitle: 'Coleccionables Digitales',
    marketplace_total_athletes: 'Atletas Totales',
    marketplace_available: 'Tokens Disponibles',
    marketplace_floor: 'Precio Base',
    marketplace_loading: 'CARGANDO TOKENS...',
    marketplace_no_match: 'Ningún token coincide con tus filtros',
    
    // Attendance Type
    attendance_title: 'ELIGE TIPO DE ASISTENCIA',
    attendance_in_person: 'EN PERSONA',
    attendance_in_person_desc: 'Asiste al evento en persona en {location}. Recibe entrada física con asiento asignado y acceso al lugar.',
    attendance_online: 'EN LÍNEA',
    attendance_online_desc: 'Ve la transmisión en vivo de forma remota. Recibe pase digital y acceso a funciones en línea exclusivas.',
    attendance_select: 'SELECCIONAR',

    // Secondary Market
    secondary_subtitle: 'Comercio de Igual a Igual',
    secondary_title: 'MERCADO SECUNDARIO',
    secondary_description: 'Compra y vende tokens de atletas y NFTs directamente de otros coleccionistas. 5% de comisión de plataforma en todos los intercambios.',
    secondary_list_for_sale: 'Listar para Vender',
    secondary_no_listings: 'Sin listados disponibles',
    secondary_quantity: 'Cantidad',
    secondary_per_unit: 'por unidad',
    secondary_total: 'Total',
    secondary_processing: 'Procesando...',
    secondary_buy_now: 'Comprar Ahora',
    secondary_create_listing: 'Crear Listado',
    secondary_select_asset: 'Seleccionar Asset para Vender',
    secondary_listing_price: 'Precio de Listado (EUR)',
    secondary_enter_price: 'Ingresa precio por unidad',
    secondary_sale_price: 'Precio de Venta',
    secondary_platform_fee: 'Comisión de Plataforma (5%)',
    secondary_you_receive: 'Recibirás',
    secondary_list_success: '¡Listado en el marketplace!',
    secondary_list_error: 'Error al crear el listado',
    secondary_purchase_success: '¡Compra exitosa!',
    secondary_purchase_error: 'Error en la compra',
    secondary_fill_fields: 'Completa todos los campos',
    secondary_cancel: 'Cancelar',
    secondary_creating: 'Creando...',
    secondary_create: 'Crear Listado',
    secondary_top_assets: 'PRINCIPALES ACTIVOS',
    secondary_recent_transactions: 'TRANSACCIONES RECIENTES',
    secondary_activity_empty: 'Sin actividad aún',
    secondary_transactions_empty: 'Sin transacciones aún',

    // Gamification
    gamification_subtitle: 'Centro de Participación de Fans',

    // Global Leaderboard
    leaderboard_subtitle: 'Sistema de Ranking Global',
    leaderboard_title: 'TABLA DE CAMPEONES',
    leaderboard_showing: 'Mostrando',
    leaderboard_empty: 'Ningún atleta en el ranking aún',
    leaderboard_loading: 'Cargando ranking...',

    my_subscriptions: 'Mis Suscripciones',
    analytics_admin: 'ANALÍTICAS ADMIN',
    analytics_personal: 'ANALÍTICAS PERSONALES',
    analytics_title: 'MÉTRICAS DE RENDIMIENTO',
    analytics_admin_desc: 'Monitorea el compromiso de la plataforma, tendencias de votación y rendimiento de apuestas',
    analytics_personal_desc: 'Rastrea tu compromiso, precisión de votación y rendimiento de apuestas en el tiempo',
    analytics_engagement: 'Compromiso',
    analytics_voting: 'Votaciones',
    analytics_betting: 'Apuestas',
    voting_hub_title: 'Hub de Votaciones',
    voting_no_live: 'No hay eventos en vivo disponibles',
    voting_view: 'Ver Votos →',
    voting_back: '← Volver a Eventos',
    voting_campaigns: 'Campañas de Votación',
    voting_leaderboard: 'Clasificación',
    voting_manage: 'Gestionar Campañas',
    voting_no_campaigns: 'No hay campañas de votación activas',
    nft_dashboard_portfolio: '// PORTAFOLIO //',
    nft_dashboard_title: 'COLECCIÓN NFT',
    nft_dashboard_total_value: 'Valor Total',
    nft_dashboard_nfts: 'NFTs',
    nft_dashboard_invested: 'Total Invertido',
    nft_dashboard_purchase: 'Precio compra',
    nft_dashboard_profit: 'Ganancia / Pérdida',
    nft_dashboard_transactions: 'Transacciones',
    nft_dashboard_buy_sell: 'Compra & Venta',
    nft_dashboard_my_nfts: 'MIS NFTs',
    nft_dashboard_sort_value: 'Ordenar por Valor',
    nft_dashboard_sort_profit: 'Ordenar por Ganancia',
    nft_dashboard_sort_recent: 'Ordenar por Reciente',
    nft_dashboard_empty: 'Aún no tienes NFTs',
    nft_dashboard_purchase_label: 'COMPRA',
    nft_dashboard_current: 'ACTUAL',
    nft_dashboard_pl: 'G/P',
    nft_dashboard_minted: 'ACUÑADO',
    nft_dashboard_return: 'Retorno',
    nft_dashboard_last_trade: 'Último Intercambio',
    nft_dashboard_history: 'HISTORIAL DE TRANSACCIONES',
    nft_dashboard_card: 'CARTA',
    nft_dashboard_type: 'TIPO',
    nft_dashboard_price: 'PRECIO',
    nft_dashboard_date: 'FECHA',
    athlete_edit_profile: 'EDITAR PERFIL',
    athlete_events_stat: 'Eventos',
    athlete_wins_stat: 'Victorias',
    athlete_podiums_stat: 'Podios',
    athlete_rating_stat: 'Puntuación',
    athlete_universal_score: 'PUNTUACIÓN UNIVERSAL',
    athlete_badges: 'INSIGNIAS',
    athlete_royalty: 'GANANCIAS ROYALTY',
    athlete_event_history: 'HISTORIAL EVENTOS',
    athlete_no_events: 'SIN EVENTOS AÚN',
    athlete_achievements: 'LOGROS',
    athlete_not_found: 'ATLETA NO ENCONTRADO',
    athlete_invalid_link: 'Enlace de perfil no válido',
    enhanced_loading: 'Cargando perfil...',
    enhanced_ugc_label: 'Envíos UGC',
    enhanced_engagement_label: 'Compromiso Total',
    enhanced_points_label: 'Puntos Ganados',
    enhanced_tokens_label: 'Tokens Poseídos',
    enhanced_tab_overview: 'resumen',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'insignias',
    enhanced_tab_events: 'eventos',
    enhanced_tab_betting: 'apuestas',
    enhanced_ugc_title: 'HISTORIAL UGC',
    enhanced_no_ugc: 'Sin envíos UGC aún',
    enhanced_badges_title: 'LOGROS E INSIGNIAS',
    enhanced_events_title: 'EVENTOS GUARDADOS',
    enhanced_no_events: 'Sin eventos guardados aún',
    enhanced_creator_level: 'Nivel Creador',
    enhanced_engagement_score: 'Puntuación Compromiso',
    enhanced_rewards_unlocked: 'Recompensas Desbloqueadas',
  },
  
  fr: {
    // Navbar
    nav_events: 'Événements',
    nav_tokens: '🎫 Jetons',
    nav_bet: '🎯 Parier',
    nav_social: '🔗 Social',
    nav_explore: 'Explorer',
    nav_tools: 'Outils',
    nav_account: 'Compte',
    nav_my_profile: 'Mon Profil',
    nav_watchlist: 'Liste de Suivi',
    nav_notifications: 'Notifications',
    nav_admin: 'Panneau Admin',
    nav_navigation: 'Navigation',
    nav_settings: 'Paramètres',
    nav_language: 'Langue',
    nav_analytics: 'Analytiques',
    nav_nft_portfolio: 'Portfolio NFT',
    nav_web3: 'Web3 & Wallet',
    
    // HUD
    hud_system: 'Système en Ligne',
    hud_ai: 'Secrétaire IA : Active',
    
    // Hero
    hero_eyebrow: '// Plateforme Sportive Mondiale //',
    hero_tagline: 'Tournois · Scouting · Communauté · 13–30',
    hero_cta_athlete: 'REJOINDRE COMME ATHLÈTE',
    hero_cta_spectator: 'REGARDER COMME SPECTATEUR',
    hero_cta_tokens: '🎫 OBTENIR JETONS',
    hero_scroll: '↓ DÉFILER POUR ÉVÉNEMENTS',
    
    // Events
    events_title: 'ÉVÉNEMENTS',
    events_subtitle: 'Circuit en Direct et Numérique',
    events_loading: 'CHARGEMENT...',
    events_empty: 'Pas encore d\'événements.',
    event_spots: 'places',
    event_live: 'EN DIRECT',
    event_ended: 'TERMINÉ',
    event_register_athlete: '🏅 Inscrire Athlète',
    event_register_spectator: '🎫 Obtenir Billet',
    event_watch_stream: '▶️ REGARDER EN DIRECT',
    event_watch_vod: '📺 VOIR REPLAY',
    
    // Registration Flow
    reg_step_info: 'Info',
    reg_step_contract: 'Contrat',
    reg_step_signature: 'Signature',
    reg_athlete_title: 'Inscription Athlète',
    reg_spectator_title: 'Billet Spectateur',
    reg_first_name: 'Prénom',
    reg_last_name: 'Nom',
    reg_email: 'Email',
    reg_phone: 'Téléphone',
    reg_dob: 'Date de Naissance',
    reg_sport: 'Discipline Sportive',
    reg_id_upload: 'Télécharger Document',
    reg_id_hint: 'JPG, PNG ou PDF - Max 5Mo',
    reg_contract_title: 'Décharge de Responsabilité',
    reg_contract_accept: 'J\'ai lu et accepte les termes',
    reg_signature_title: 'Signature Numérique',
    reg_signature_hint: 'Signez avec votre doigt ou souris',
    reg_signature_clear: 'Effacer',
    reg_back: 'Retour',
    reg_next: 'Suivant',
    reg_submit: 'Soumettre',
    reg_submitting: 'Traitement...',
    reg_close: '✕ FERMER',
    reg_upload_error: 'Échec du téléchargement.',
    reg_upload_size: 'Fichier trop volumineux. Max 5Mo.',
    
    // Success Modal
    success_athlete_title: 'ATHLÈTE INSCRIT',
    success_spectator_title: 'BILLET CONFIRMÉ',
    success_athlete_msg: 'Votre inscription est confirmée. Vérifiez votre email.',
    success_spectator_msg: 'Votre billet est prêt. Vérifiez votre email.',
    success_ticket: 'VOTRE BILLET',
    success_seat: 'Siège',
    success_zone: 'Zone',
    success_close: 'COMPRIS',
    
    // Tokens
    tokens_title: 'JETONS D\'ATHLÈTE',
    tokens_subtitle: 'Objets Numériques',
    tokens_intro: 'Soutenez vos athlètes préférés et débloquez des avantages exclusifs.',
    tokens_soon: 'Marketplace de jetons bientôt.',
    tokens_cta: 'REJOINDRE LISTE',
    
    // Betting
    bet_title: 'PARIS ÉTHIQUES',
    bet_subtitle: 'Récompenses de Prédiction',
    bet_locked_title: 'Jeton Requis',
    bet_locked_msg: 'Achetez un jeton d\'athlète pour débloquer les défis.',
    bet_locked_cta: 'Obtenir Jetons',
    bet_my_bets: 'MES PRÉDICTIONS',
    bet_empty: 'Pas de paris actifs.',
    bet_how_title: 'Comment ça Marche',
    bet_how_1: '1. Possédez un jeton',
    bet_how_2: '2. Prédisez les résultats',
    bet_how_3: '3. Gagnez des prix',
    bet_how_4: '4. Réclamez les récompenses',
    bet_disclaimer: '⚠️ AUCUN PRIX EN ARGENT. Système de gamification éthique.',
    
    // Social Rewards
    social_rewards_title: 'AVANTAGES SPECTATEUR',
    social_rewards_subtitle: 'Suivez et Gagnez',
    social_rewards_msg: 'Pas de jeton ? Suivez-nous pour des cadeaux gratuits.',
    social_rewards_list: '✓ Suivez les 5 comptes\n✓ Montrez la preuve\n✓ Obtenez des cadeaux',
    social_rewards_cta: 'VOIR LIENS SOCIAUX',
    
    // Sponsor
    sponsor_title: 'PARTENARIAT',
    sponsor_subtitle: 'Atteindre la Nouvelle Génération',
    sponsor_value: 'Street Dinamics connecte votre marque avec des athlètes et des fans de 13–30 ans à l\'échelle mondiale. Visibilité multi-plateformes, activation d\'événements en direct, NFT drops et engagement authentique aux EAU, en Europe et sur les marchés internationaux.',
    sponsor_demo: 'Âges 13–30 • 50K+ portée mondiale • Événements en direct et numériques • Intégration de tokens',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'Réponse sous 48h. Kits média disponibles sur demande.',
    
    // Social Modal
    social_modal_title: 'STREET DINAMICS',
    social_modal_subtitle: 'Suivez-nous Partout',
    social_instagram: 'Instagram',
    social_tiktok: 'TikTok',
    social_youtube: 'YouTube',
    social_kick: 'Kick — Diffusions',
    social_snapchat: 'Snapchat',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, Dubai, UAE) — Tous Droits Réservés',
    
    // Onboarding
    onboard_welcome: 'BIENVENUE À STREET DINAMICS',
    onboard_choose: 'Choisissez Votre Voie',
    onboard_athlete: 'ATHLÈTE',
    onboard_athlete_desc: 'Participez aux événements, construisez votre profil, gagnez des jetons et développez votre base de fans',
    onboard_spectator: 'SPECTATEUR',
    onboard_spectator_desc: 'Regardez les événements, soutenez les athlètes, collectez des jetons et gagnez des prix',
    onboard_profile_title: 'Complétez Votre Profil',
    onboard_photo: 'Télécharger Photo',
    onboard_phone: 'Téléphone',
    onboard_dob: 'Date de Naissance',
    onboard_country: 'Pays',
    onboard_city: 'Ville',
    onboard_sports: 'Sports (séparés par virgule)',
    onboard_sports_placeholder: 'ex. Parkour, BMX, Skateboarding',
    onboard_bio: 'Bio',
    onboard_bio_placeholder: 'Parlez-nous de vous et de votre parcours...',
    onboard_fav_sports: 'Sports Favoris (séparés par virgule)',
    onboard_back: 'Retour',
    onboard_complete: 'Terminer la Configuration',
    onboard_creating: 'Création du Profil...',
    
    // Admin
    admin_panel: 'PANNEAU ADMIN',
    admin_create_event: '+ CRÉER ÉVÉNEMENT',
    admin_go_live: 'PASSER EN DIRECT',
    admin_end_event: 'TERMINER ÉVÉNEMENT',
    admin_kick_url: 'URL Live Kick',
    admin_youtube_url: 'URL Live YouTube',
    admin_save_links: 'Enregistrer Liens Live',
    admin_change_pass: 'Changer Mot de Passe',
    admin_new_pass: 'Nouveau Mot de Passe',
    admin_confirm_pass: 'Confirmer Mot de Passe',
    
    // Token Marketplace
    marketplace_title: 'MARKETPLACE DE JETONS',
    marketplace_subtitle: 'Objets Numériques',
    marketplace_total_athletes: 'Athlètes Totaux',
    marketplace_available: 'Jetons Disponibles',
    marketplace_floor: 'Prix Plancher',
    marketplace_loading: 'CHARGEMENT DES JETONS...',
    marketplace_no_match: 'Aucun jeton ne correspond à vos filtres',
    
    // Attendance Type
    attendance_title: 'CHOISIR LE TYPE DE PARTICIPATION',
    attendance_in_person: 'EN PERSONNE',
    attendance_in_person_desc: 'Assistez à l\'événement en personne à {location}. Recevez un billet physique avec siège assigné et accès au lieu.',
    attendance_online: 'EN LIGNE',
    attendance_online_desc: 'Regardez la diffusion en direct à distance. Recevez un pass numérique et accès à des fonctionnalités en ligne exclusives.',
    attendance_select: 'SÉLECTIONNER',

    // Secondary Market
    secondary_subtitle: 'Trading de Pair à Pair',
    secondary_title: 'MARCHÉ SECONDAIRE',
    secondary_description: 'Achetez et vendez des tokens d\'athlètes et des NFT directement auprès d\'autres collectionneurs. 5% de frais de plateforme sur tous les échanges.',
    secondary_list_for_sale: 'Mettre en Vente',
    secondary_no_listings: 'Aucune annonce disponible',
    secondary_quantity: 'Quantité',
    secondary_per_unit: 'par unité',
    secondary_total: 'Total',
    secondary_processing: 'Traitement...',
    secondary_buy_now: 'Acheter Maintenant',
    secondary_create_listing: 'Créer une Annonce',
    secondary_select_asset: 'Sélectionner Asset à Vendre',
    secondary_listing_price: 'Prix de l\'Annonce (EUR)',
    secondary_enter_price: 'Entrez le prix par unité',
    secondary_sale_price: 'Prix de Vente',
    secondary_platform_fee: 'Frais de Plateforme (5%)',
    secondary_you_receive: 'Vous Recevrez',
    secondary_list_success: 'Mis en vente sur la marketplace !',
    secondary_list_error: 'Impossible de créer l\'annonce',
    secondary_purchase_success: 'Achat réussi !',
    secondary_purchase_error: 'Achat échoué',
    secondary_fill_fields: 'Remplissez tous les champs',
    secondary_cancel: 'Annuler',
    secondary_creating: 'Création en cours...',
    secondary_create: 'Créer une Annonce',
    secondary_top_assets: 'ACTIFS PRINCIPAUX',
    secondary_recent_transactions: 'TRANSACTIONS RÉCENTES',
    secondary_activity_empty: 'Pas d\'activité encore',
    secondary_transactions_empty: 'Pas de transactions encore',

    // Gamification
    gamification_subtitle: 'Hub d\'Engagement des Fans',

    // Global Leaderboard
    leaderboard_subtitle: 'Système de Classement Global',
    leaderboard_title: 'TABLEAU DES CHAMPIONS',
    leaderboard_showing: 'Affichage',
    leaderboard_empty: 'Aucun athlète sur le classement encore',
    leaderboard_loading: 'Chargement du classement...',

    my_subscriptions: 'Mes Abonnements',
    analytics_admin: 'ANALYTIQUES ADMIN',
    analytics_personal: 'ANALYTIQUES PERSONNELLES',
    analytics_title: 'MÉTRIQUES DE PERFORMANCE',
    analytics_admin_desc: 'Surveillez l\'engagement de la plateforme, les tendances de vote et les performances des paris',
    analytics_personal_desc: 'Suivez votre engagement, la précision de vote et les performances des paris',
    analytics_engagement: 'Engagement',
    analytics_voting: 'Votes',
    analytics_betting: 'Paris',
    voting_hub_title: 'Hub de Vote',
    voting_no_live: 'Aucun événement en direct',
    voting_view: 'Voir les Votes →',
    voting_back: '← Retour aux Événements',
    voting_campaigns: 'Campagnes de Vote',
    voting_leaderboard: 'Classement',
    voting_manage: 'Gérer Campagnes',
    voting_no_campaigns: 'Aucune campagne de vote active',
    nft_dashboard_portfolio: '// PORTEFEUILLE //',
    nft_dashboard_title: 'COLLECTION NFT',
    nft_dashboard_total_value: 'Valeur Totale',
    nft_dashboard_nfts: 'NFTs',
    nft_dashboard_invested: 'Total Investi',
    nft_dashboard_purchase: 'Prix d\'achat',
    nft_dashboard_profit: 'Profit / Perte',
    nft_dashboard_transactions: 'Transactions',
    nft_dashboard_buy_sell: 'Achat & Vente',
    nft_dashboard_my_nfts: 'MES NFTs',
    nft_dashboard_sort_value: 'Trier par Valeur',
    nft_dashboard_sort_profit: 'Trier par Profit',
    nft_dashboard_sort_recent: 'Trier par Récent',
    nft_dashboard_empty: 'Vous ne possédez pas encore de NFT',
    nft_dashboard_purchase_label: 'ACHAT',
    nft_dashboard_current: 'ACTUEL',
    nft_dashboard_pl: 'P/P',
    nft_dashboard_minted: 'FRAPPÉ',
    nft_dashboard_return: 'Rendement',
    nft_dashboard_last_trade: 'Dernier Échange',
    nft_dashboard_history: 'HISTORIQUE TRANSACTIONS',
    nft_dashboard_card: 'CARTE',
    nft_dashboard_type: 'TYPE',
    nft_dashboard_price: 'PRIX',
    nft_dashboard_date: 'DATE',
    athlete_edit_profile: 'MODIFIER PROFIL',
    athlete_events_stat: 'Événements',
    athlete_wins_stat: 'Victoires',
    athlete_podiums_stat: 'Podiums',
    athlete_rating_stat: 'Note',
    athlete_universal_score: 'SCORE UNIVERSEL',
    athlete_badges: 'BADGES',
    athlete_royalty: 'GAINS ROYALTIES',
    athlete_event_history: 'HISTORIQUE ÉVÉNEMENTS',
    athlete_no_events: 'AUCUN ÉVÉNEMENT ENCORE',
    athlete_achievements: 'RÉALISATIONS',
    athlete_not_found: 'ATHLÈTE NON TROUVÉ',
    athlete_invalid_link: 'Lien de profil invalide',
    enhanced_loading: 'Chargement du profil...',
    enhanced_ugc_label: 'Soumissions UGC',
    enhanced_engagement_label: 'Engagement Total',
    enhanced_points_label: 'Points Gagnés',
    enhanced_tokens_label: 'Jetons Possédés',
    enhanced_tab_overview: 'aperçu',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'badges',
    enhanced_tab_events: 'événements',
    enhanced_tab_betting: 'paris',
    enhanced_ugc_title: 'HISTORIQUE UGC',
    enhanced_no_ugc: 'Aucune soumission UGC encore',
    enhanced_badges_title: 'RÉALISATIONS & BADGES',
    enhanced_events_title: 'ÉVÉNEMENTS SAUVEGARDÉS',
    enhanced_no_events: 'Aucun événement sauvegardé',
    enhanced_creator_level: 'Niveau Créateur',
    enhanced_engagement_score: 'Score Engagement',
    enhanced_rewards_unlocked: 'Récompenses Débloquées',
  },
  
  de: {
    // Navbar
    nav_events: 'Veranstaltungen',
    nav_tokens: '🎫 Token',
    nav_bet: '🎯 Wetten',
    nav_social: '🔗 Social',
    nav_explore: 'Erkunden',
    nav_tools: 'Werkzeuge',
    nav_account: 'Konto',
    nav_my_profile: 'Mein Profil',
    nav_watchlist: 'Beobachtungsliste',
    nav_notifications: 'Benachrichtigungen',
    nav_admin: 'Admin-Panel',
    nav_navigation: 'Navigation',
    nav_settings: 'Einstellungen',
    nav_language: 'Sprache',
    nav_analytics: 'Analytik',
    nav_nft_portfolio: 'NFT-Portfolio',
    nav_web3: 'Web3 & Wallet',
    
    // HUD
    hud_system: 'System Online',
    hud_ai: 'KI-Sekretärin: Aktiv',
    
    // Hero
    hero_eyebrow: '// Globale Sport-Plattform //',
    hero_tagline: 'Turniere · Scouting · Community · 13–30',
    hero_cta_athlete: 'ALS ATHLET BEITRETEN',
    hero_cta_spectator: 'ALS ZUSCHAUER ANSEHEN',
    hero_cta_tokens: '🎫 TOKEN ERHALTEN',
    hero_scroll: '↓ SCROLLEN FÜR EVENTS',
    
    // Events
    events_title: 'VERANSTALTUNGEN',
    events_subtitle: 'Live- und Digital-Circuit',
    events_loading: 'LADE EVENTS...',
    events_empty: 'Noch keine Events.',
    event_spots: 'Plätze',
    event_live: 'LIVE JETZT',
    event_ended: 'BEENDET',
    event_register_athlete: '🏅 Athlet Anmelden',
    event_register_spectator: '🎫 Ticket Holen',
    event_watch_stream: '▶️ LIVE ANSEHEN',
    event_watch_vod: '📺 WIEDERHOLUNG',
    
    // Registration Flow
    reg_step_info: 'Info',
    reg_step_contract: 'Vertrag',
    reg_step_signature: 'Unterschrift',
    reg_athlete_title: 'Athleten-Anmeldung',
    reg_spectator_title: 'Zuschauer-Ticket',
    reg_first_name: 'Vorname',
    reg_last_name: 'Nachname',
    reg_email: 'E-Mail',
    reg_phone: 'Telefon',
    reg_dob: 'Geburtsdatum',
    reg_sport: 'Sportdisziplin',
    reg_id_upload: 'Dokument Hochladen',
    reg_id_hint: 'JPG, PNG oder PDF - Max 5MB',
    reg_contract_title: 'Haftungsausschluss',
    reg_contract_accept: 'Ich habe gelesen und akzeptiere die Bedingungen',
    reg_signature_title: 'Digitale Unterschrift',
    reg_signature_hint: 'Mit Finger oder Maus unterschreiben',
    reg_signature_clear: 'Löschen',
    reg_back: 'Zurück',
    reg_next: 'Weiter',
    reg_submit: 'Absenden',
    reg_submitting: 'Verarbeitung...',
    reg_close: '✕ SCHLIESSEN',
    reg_upload_error: 'Upload fehlgeschlagen.',
    reg_upload_size: 'Datei zu groß. Max 5MB.',
    
    // Success Modal
    success_athlete_title: 'ATHLET ANGEMELDET',
    success_spectator_title: 'TICKET BESTÄTIGT',
    success_athlete_msg: 'Ihre Anmeldung ist bestätigt. Prüfen Sie Ihre E-Mail.',
    success_spectator_msg: 'Ihr Ticket ist bereit. Prüfen Sie Ihre E-Mail.',
    success_ticket: 'IHR TICKET',
    success_seat: 'Sitzplatz',
    success_zone: 'Zone',
    success_close: 'VERSTANDEN',
    
    // Tokens
    tokens_title: 'ATHLETEN-TOKEN',
    tokens_subtitle: 'Digitale Sammlerstücke',
    tokens_intro: 'Unterstützen Sie Ihre Lieblingsathleten und entsperren Sie exklusive Vorteile.',
    tokens_soon: 'Token-Marktplatz bald verfügbar.',
    tokens_cta: 'WARTELISTE BEITRETEN',
    
    // Betting
    bet_title: 'ETHISCHES WETTEN',
    bet_subtitle: 'Vorhersage-Belohnungen',
    bet_locked_title: 'Token Erforderlich',
    bet_locked_msg: 'Kaufen Sie einen Athleten-Token, um Herausforderungen freizuschalten.',
    bet_locked_cta: 'Token Erhalten',
    bet_my_bets: 'MEINE VORHERSAGEN',
    bet_empty: 'Keine aktiven Wetten.',
    bet_how_title: 'Wie es Funktioniert',
    bet_how_1: '1. Besitzen Sie einen Token',
    bet_how_2: '2. Vorhersagen Sie Ergebnisse',
    bet_how_3: '3. Gewinnen Sie Preise',
    bet_how_4: '4. Belohnungen einlösen',
    bet_disclaimer: '⚠️ KEINE GELDPREISE. Ethisches Gamification-System.',
    
    // Social Rewards
    social_rewards_title: 'ZUSCHAUER-VORTEILE',
    social_rewards_subtitle: 'Folgen und Gewinnen',
    social_rewards_msg: 'Kein Token? Folgen Sie uns für kostenlose Geschenke.',
    social_rewards_list: '✓ Folgen Sie allen 5 Konten\n✓ Nachweis zeigen\n✓ Geschenke erhalten',
    social_rewards_cta: 'SOCIAL-LINKS ANSEHEN',
    
    // Sponsor
    sponsor_title: 'PARTNERSCHAFT',
    sponsor_subtitle: 'Erreichen Sie die Neue Generation',
    sponsor_value: 'Street Dinamics verbindet Ihre Marke mit 13–30-jährigen Athleten und Fans weltweit. Multi-Plattform-Sichtbarkeit, Live-Event-Aktivierung, NFT-Drops und authentisches Community-Engagement in den VAE, Europa und internationalen Märkten.',
    sponsor_demo: 'Alter 13–30 • 50K+ globale Reichweite • Live- und digitale Events • Token-Integration',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'Antwort innerhalb 48h. Medien-Kits auf Anfrage verfügbar.',
    
    // Social Modal
    social_modal_title: 'STREET DINAMICS',
    social_modal_subtitle: 'Folgen Sie Uns Überall',
    social_instagram: 'Instagram',
    social_tiktok: 'TikTok',
    social_youtube: 'YouTube',
    social_kick: 'Kick — Live-Streams',
    social_snapchat: 'Snapchat',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, Dubai, UAE) — Alle Rechte Vorbehalten',
    
    // Onboarding
    onboard_welcome: 'WILLKOMMEN BEI STREET DINAMICS',
    onboard_choose: 'Wählen Sie Ihren Weg',
    onboard_athlete: 'ATHLET',
    onboard_athlete_desc: 'Nimm an Events teil, baue dein Profil auf, verdiene Token und vergrößere deine Fanbasis',
    onboard_spectator: 'ZUSCHAUER',
    onboard_spectator_desc: 'Schaue Events, unterstütze Athleten, sammle Token und gewinne Preise',
    onboard_profile_title: 'Vervollständige Dein Profil',
    onboard_photo: 'Foto Hochladen',
    onboard_phone: 'Telefon',
    onboard_dob: 'Geburtsdatum',
    onboard_country: 'Land',
    onboard_city: 'Stadt',
    onboard_sports: 'Sportarten (durch Komma getrennt)',
    onboard_sports_placeholder: 'z.B. Parkour, BMX, Skateboarding',
    onboard_bio: 'Biografie',
    onboard_bio_placeholder: 'Erzähle uns über dich und deine Reise...',
    onboard_fav_sports: 'Lieblingssportarten (durch Komma getrennt)',
    onboard_back: 'Zurück',
    onboard_complete: 'Setup Abschließen',
    onboard_creating: 'Profil Wird Erstellt...',
    
    // Admin
    admin_panel: 'ADMIN-PANEL',
    admin_create_event: '+ EVENT ERSTELLEN',
    admin_go_live: 'LIVE GEHEN',
    admin_end_event: 'EVENT BEENDEN',
    admin_kick_url: 'Kick Live URL',
    admin_youtube_url: 'YouTube Live URL',
    admin_save_links: 'Live-Links Speichern',
    admin_change_pass: 'Passwort Ändern',
    admin_new_pass: 'Neues Passwort',
    admin_confirm_pass: 'Passwort Bestätigen',
    
    // Token Marketplace
    marketplace_title: 'TOKEN-MARKTPLATZ',
    marketplace_subtitle: 'Digitale Sammlerstücke',
    marketplace_total_athletes: 'Athleten Gesamt',
    marketplace_available: 'Verfügbare Token',
    marketplace_floor: 'Mindestpreis',
    marketplace_loading: 'LADE TOKEN...',
    marketplace_no_match: 'Keine Token entsprechen Ihren Filtern',
    
    // Attendance Type
    attendance_title: 'TEILNAHMEART WÄHLEN',
    attendance_in_person: 'VOR ORT',
    attendance_in_person_desc: 'Nehmen Sie persönlich an der Veranstaltung in {location} teil. Erhalten Sie ein physisches Ticket mit zugewiesenem Sitzplatz und Zugang zum Veranstaltungsort.',
    attendance_online: 'ONLINE',
    attendance_online_desc: 'Sehen Sie sich den Livestream aus der Ferne an. Erhalten Sie einen digitalen Pass und Zugang zu exklusiven Online-Funktionen.',
    attendance_select: 'AUSWÄHLEN',

    // Secondary Market
    secondary_subtitle: 'Peer-to-Peer-Handel',
    secondary_title: 'SEKUNDÄRMARKT',
    secondary_description: 'Kaufen und verkaufen Sie Athleten-Token und NFTs direkt von anderen Sammlern. 5% Plattformgebühr auf alle Transaktionen.',
    secondary_list_for_sale: 'Zum Verkauf Anbieten',
    secondary_no_listings: 'Keine Angebote verfügbar',
    secondary_quantity: 'Menge',
    secondary_per_unit: 'pro Einheit',
    secondary_total: 'Gesamt',
    secondary_processing: 'Verarbeitung...',
    secondary_buy_now: 'Jetzt Kaufen',
    secondary_create_listing: 'Angebot Erstellen',
    secondary_select_asset: 'Asset zum Verkauf Auswählen',
    secondary_listing_price: 'Angebotspreis (EUR)',
    secondary_enter_price: 'Preis pro Einheit eingeben',
    secondary_sale_price: 'Verkaufspreis',
    secondary_platform_fee: 'Plattformgebühr (5%)',
    secondary_you_receive: 'Sie Erhalten',
    secondary_list_success: 'Auf dem Marktplatz aufgelistet!',
    secondary_list_error: 'Angebot konnte nicht erstellt werden',
    secondary_purchase_success: 'Kauf erfolgreich!',
    secondary_purchase_error: 'Kauf fehlgeschlagen',
    secondary_fill_fields: 'Füllen Sie alle Felder aus',
    secondary_cancel: 'Abbrechen',
    secondary_creating: 'Erstellen läuft...',
    secondary_create: 'Angebot Erstellen',
    secondary_top_assets: 'TOP ASSETS',
    secondary_recent_transactions: 'AKTUELLE TRANSAKTIONEN',
    secondary_activity_empty: 'Noch keine Aktivität',
    secondary_transactions_empty: 'Noch keine Transaktionen',

    // Gamification
    gamification_subtitle: 'Fan-Engagement-Hub',

    // Global Leaderboard
    leaderboard_subtitle: 'Globales Ranking-System',
    leaderboard_title: 'CHAMPIONS BOARD',
    leaderboard_showing: 'Anzeige',
    leaderboard_empty: 'Noch keine Athleten auf der Rangliste',
    leaderboard_loading: 'Rankings werden geladen...',

    my_subscriptions: 'Meine Abonnements',
    analytics_admin: 'ADMIN-ANALYTIK',
    analytics_personal: 'PERSÖNLICHE ANALYTIK',
    analytics_title: 'LEISTUNGSMETRIKEN',
    analytics_admin_desc: 'Plattformweites Engagement, Abstimmungstrends und Wettleistung überwachen',
    analytics_personal_desc: 'Verfolgen Sie Ihr Engagement, Abstimmungsgenauigkeit und Wettleistung',
    analytics_engagement: 'Engagement',
    analytics_voting: 'Abstimmung',
    analytics_betting: 'Wetten',
    voting_hub_title: 'Abstimmungs-Hub',
    voting_no_live: 'Keine Live-Veranstaltungen verfügbar',
    voting_view: 'Stimmen anzeigen →',
    voting_back: '← Zurück zu Events',
    voting_campaigns: 'Abstimmungskampagnen',
    voting_leaderboard: 'Rangliste',
    voting_manage: 'Kampagnen verwalten',
    voting_no_campaigns: 'Keine aktiven Abstimmungskampagnen',
    nft_dashboard_portfolio: '// PORTFOLIO //',
    nft_dashboard_title: 'NFT-SAMMLUNG',
    nft_dashboard_total_value: 'Gesamtwert',
    nft_dashboard_nfts: 'NFTs',
    nft_dashboard_invested: 'Gesamt Investiert',
    nft_dashboard_purchase: 'Kaufpreis',
    nft_dashboard_profit: 'Gewinn / Verlust',
    nft_dashboard_transactions: 'Transaktionen',
    nft_dashboard_buy_sell: 'Kauf & Verkauf',
    nft_dashboard_my_nfts: 'MEINE NFTs',
    nft_dashboard_sort_value: 'Nach Wert sortieren',
    nft_dashboard_sort_profit: 'Nach Gewinn sortieren',
    nft_dashboard_sort_recent: 'Nach Datum sortieren',
    nft_dashboard_empty: 'Sie besitzen noch keine NFTs',
    nft_dashboard_purchase_label: 'KAUF',
    nft_dashboard_current: 'AKTUELL',
    nft_dashboard_pl: 'G/V',
    nft_dashboard_minted: 'GEMINT',
    nft_dashboard_return: 'Rendite',
    nft_dashboard_last_trade: 'Letzter Handel',
    nft_dashboard_history: 'TRANSAKTIONSHISTORIE',
    nft_dashboard_card: 'KARTE',
    nft_dashboard_type: 'TYP',
    nft_dashboard_price: 'PREIS',
    nft_dashboard_date: 'DATUM',
    athlete_edit_profile: 'PROFIL BEARBEITEN',
    athlete_events_stat: 'Events',
    athlete_wins_stat: 'Siege',
    athlete_podiums_stat: 'Podien',
    athlete_rating_stat: 'Bewertung',
    athlete_universal_score: 'UNIVERSAL SCORE',
    athlete_badges: 'ABZEICHEN',
    athlete_royalty: 'LIZENZEINNAHMEN',
    athlete_event_history: 'EVENT-VERLAUF',
    athlete_no_events: 'NOCH KEINE EVENTS',
    athlete_achievements: 'ERFOLGE',
    athlete_not_found: 'ATHLET NICHT GEFUNDEN',
    athlete_invalid_link: 'Ungültiger Profillink',
    enhanced_loading: 'Profil wird geladen...',
    enhanced_ugc_label: 'UGC-Einsendungen',
    enhanced_engagement_label: 'Gesamt-Engagement',
    enhanced_points_label: 'Verdiente Punkte',
    enhanced_tokens_label: 'Besessene Token',
    enhanced_tab_overview: 'übersicht',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'abzeichen',
    enhanced_tab_events: 'events',
    enhanced_tab_betting: 'wetten',
    enhanced_ugc_title: 'UGC-EINSENDUNGSHISTORIE',
    enhanced_no_ugc: 'Noch keine UGC-Einsendungen',
    enhanced_badges_title: 'ERFOLGE & ABZEICHEN',
    enhanced_events_title: 'GESPEICHERTE EVENTS',
    enhanced_no_events: 'Noch keine gespeicherten Events',
    enhanced_creator_level: 'Creator-Level',
    enhanced_engagement_score: 'Engagement-Score',
    enhanced_rewards_unlocked: 'Belohnungen freigeschaltet',
  },
  
  ar: {
    // Navbar
    nav_events: 'الفعاليات',
    nav_tokens: '🎫 الرموز',
    nav_bet: '🎯 المراهنة',
    nav_social: '🔗 التواصل',
    nav_explore: 'استكشاف',
    nav_tools: 'أدوات',
    nav_account: 'الحساب',
    nav_my_profile: 'ملفي الشخصي',
    nav_watchlist: 'قائمة المتابعة',
    nav_notifications: 'الإشعارات',
    nav_admin: 'لوحة الإدارة',
    nav_navigation: 'التنقل',
    nav_settings: 'الإعدادات',
    nav_language: 'اللغة',
    nav_analytics: 'التحليلات',
    nav_nft_portfolio: 'محفظة NFT',
    nav_web3: 'Web3 والمحفظة',
    
    // HUD
    hud_system: 'النظام متصل',
    hud_ai: 'السكرتيرة الذكية: نشطة',
    
    // Hero
    hero_eyebrow: '// منصة رياضية عالمية //',
    hero_tagline: 'البطولات · الكشف · المجتمع · 13–30',
    hero_cta_athlete: 'انضم كرياضي',
    hero_cta_spectator: 'شاهد كمتفرج',
    hero_cta_tokens: '🎫 احصل على الرموز',
    hero_scroll: '↓ مرر للفعاليات',
    
    // Events
    events_title: 'الفعاليات',
    events_subtitle: 'الدائرة المباشرة والرقمية',
    events_loading: 'جارٍ التحميل...',
    events_empty: 'لا توجد فعاليات بعد.',
    event_spots: 'أماكن',
    event_live: 'مباشر الآن',
    event_ended: 'انتهى',
    event_register_athlete: '🏅 تسجيل رياضي',
    event_register_spectator: '🎫 احصل على تذكرة',
    event_watch_stream: '▶️ مشاهدة مباشرة',
    event_watch_vod: '📺 مشاهدة الإعادة',
    
    // Registration Flow
    reg_step_info: 'المعلومات',
    reg_step_contract: 'العقد',
    reg_step_signature: 'التوقيع',
    reg_athlete_title: 'تسجيل الرياضي',
    reg_spectator_title: 'تذكرة المتفرج',
    reg_first_name: 'الاسم الأول',
    reg_last_name: 'اسم العائلة',
    reg_email: 'البريد الإلكتروني',
    reg_phone: 'الهاتف',
    reg_dob: 'تاريخ الميلاد',
    reg_sport: 'التخصص الرياضي',
    reg_id_upload: 'رفع الوثيقة',
    reg_id_hint: 'JPG أو PNG أو PDF - الحد الأقصى 5 ميجابايت',
    reg_contract_title: 'إخلاء المسؤولية',
    reg_contract_accept: 'لقد قرأت وأوافق على الشروط',
    reg_signature_title: 'التوقيع الرقمي',
    reg_signature_hint: 'وقع بإصبعك أو الماوس',
    reg_signature_clear: 'مسح',
    reg_back: 'رجوع',
    reg_next: 'التالي',
    reg_submit: 'إرسال التسجيل',
    reg_submitting: 'جارٍ المعالجة...',
    reg_close: '✕ إغلاق',
    reg_upload_error: 'فشل الرفع. حاول مرة أخرى.',
    reg_upload_size: 'الملف كبير جداً. الحد الأقصى 5 ميجابايت.',
    
    // Success Modal
    success_athlete_title: 'تم تسجيل الرياضي',
    success_spectator_title: 'تم تأكيد التذكرة',
    success_athlete_msg: 'تم تأكيد تسجيلك. تحقق من بريدك الإلكتروني.',
    success_spectator_msg: 'تذكرتك جاهزة. تحقق من بريدك الإلكتروني.',
    success_ticket: 'تذكرتك',
    success_seat: 'المقعد',
    success_zone: 'المنطقة',
    success_close: 'فهمت',
    
    // Tokens
    tokens_title: 'رموز الرياضيين',
    tokens_subtitle: 'المقتنيات الرقمية',
    tokens_intro: 'ادعم رياضييك المفضلين واحصل على امتيازات حصرية.',
    tokens_soon: 'سوق الرموز قريباً.',
    tokens_cta: 'انضم للقائمة',
    
    // Betting
    bet_title: 'المراهنة الأخلاقية',
    bet_subtitle: 'مكافآت التوقع',
    bet_locked_title: 'الرمز مطلوب',
    bet_locked_msg: 'اشترِ رمز رياضي لفتح التحديات.',
    bet_locked_cta: 'احصل على الرموز',
    bet_my_bets: 'توقعاتي',
    bet_empty: 'لا توجد رهانات نشطة.',
    bet_how_title: 'كيف يعمل',
    bet_how_1: '1. امتلك رمزاً',
    bet_how_2: '2. توقع النتائج',
    bet_how_3: '3. اربح جوائز',
    bet_how_4: '4. استبدل المكافآت',
    bet_disclaimer: '⚠️ لا توجد جوائز نقدية. نظام ألعاب أخلاقي.',
    
    // Social Rewards
    social_rewards_title: 'امتيازات المتفرجين',
    social_rewards_subtitle: 'تابع واربح',
    social_rewards_msg: 'لا يوجد رمز؟ تابعنا للحصول على هدايا مجانية.',
    social_rewards_list: '✓ تابع جميع الحسابات الـ5\n✓ أظهر الإثبات\n✓ احصل على هدايا',
    social_rewards_cta: 'عرض الروابط الاجتماعية',
    
    // Sponsor
    sponsor_title: 'كن شريكاً',
    sponsor_subtitle: 'اصل للجيل الجديد',
    sponsor_value: 'ستريت ديناميكس تربط علامتك التجارية برياضيين ومشجعين من عمر 13-30 على مستوى العالم. رؤية متعددة المنصات، تفعيل الفعاليات المباشرة، إصدارات NFT، ومشاركة مجتمعية أصيلة في الإمارات وأوروبا والأسواق العالمية.',
    sponsor_demo: 'الأعمار 13–30 • 50 ألف+ وصول عالمي • فعاليات مباشرة ورقمية • تكامل الرموز',
    sponsor_cta: 'partnerships@streetdinamics.ae',
    sponsor_note: 'نجيب خلال 48 ساعة. حزم إعلامية متاحة عند الطلب.',
    
    // Social Modal
    social_modal_title: 'ستريت ديناميكس',
    social_modal_subtitle: 'تابعنا في كل مكان',
    social_instagram: 'إنستغرام',
    social_tiktok: 'تيك توك',
    social_youtube: 'يوتيوب',
    social_kick: 'كيك — البث المباشر',
    social_snapchat: 'سناب شات',
    
    // Footer
    footer_rights: '© 2026 Street Dynamics Holding FZE (IFZA, دبي، الإمارات) — جميع الحقوق محفوظة',
    
    // Onboarding
    onboard_welcome: 'مرحباً بك في ستريت ديناميكس',
    onboard_choose: 'اختر مسارك',
    onboard_athlete: 'رياضي',
    onboard_athlete_desc: 'شارك في الفعاليات، ابنِ ملفك الشخصي، اكسب الرموز، ونمّي قاعدة معجبيك',
    onboard_spectator: 'متفرج',
    onboard_spectator_desc: 'شاهد الفعاليات، ادعم الرياضيين، اجمع الرموز، واربح الجوائز',
    onboard_profile_title: 'أكمل ملفك الشخصي',
    onboard_photo: 'رفع صورة',
    onboard_phone: 'الهاتف',
    onboard_dob: 'تاريخ الميلاد',
    onboard_country: 'البلد',
    onboard_city: 'المدينة',
    onboard_sports: 'الرياضات (مفصولة بفواصل)',
    onboard_sports_placeholder: 'مثل: باركور، BMX، التزلج',
    onboard_bio: 'السيرة الذاتية',
    onboard_bio_placeholder: 'أخبرنا عن نفسك ورحلتك...',
    onboard_fav_sports: 'الرياضات المفضلة (مفصولة بفواصل)',
    onboard_back: 'رجوع',
    onboard_complete: 'إكمال الإعداد',
    onboard_creating: 'جارٍ إنشاء الملف الشخصي...',
    
    // Admin
    admin_panel: 'لوحة الإدارة',
    admin_create_event: '+ إنشاء فعالية',
    admin_go_live: 'انطلق مباشرة',
    admin_end_event: 'إنهاء الفعالية',
    admin_kick_url: 'رابط Kick المباشر',
    admin_youtube_url: 'رابط YouTube المباشر',
    admin_save_links: 'حفظ روابط البث',
    admin_change_pass: 'تغيير كلمة المرور',
    admin_new_pass: 'كلمة المرور الجديدة',
    admin_confirm_pass: 'تأكيد كلمة المرور',
    
    // Token Marketplace
    marketplace_title: 'سوق الرموز',
    marketplace_subtitle: 'المقتنيات الرقمية',
    marketplace_total_athletes: 'إجمالي الرياضيين',
    marketplace_available: 'الرموز المتاحة',
    marketplace_floor: 'السعر الأدنى',
    marketplace_loading: 'جارٍ تحميل الرموز...',
    marketplace_no_match: 'لا توجد رموز تطابق مرشحاتك',
    
    // Attendance Type
    attendance_title: 'اختر نوع الحضور',
    attendance_in_person: 'حضور شخصي',
    attendance_in_person_desc: 'احضر الحدث شخصياً في {location}. احصل على تذكرة فعلية مع مقعد محدد والدخول إلى المكان.',
    attendance_online: 'عبر الإنترنت',
    attendance_online_desc: 'شاهد البث المباشر عن بُعد. احصل على تصريح رقمي وإمكانية الوصول إلى الميزات الحصرية عبر الإنترنت.',
    attendance_select: 'اختيار',

    // NFT Marketplace
    nft_subtitle: 'لحظات تاريخية',
    nft_title: 'بطاقات مجموعة NFT',
    nft_description: 'تلتقط كل NFT لحظة تاريخية فريدة. عرض محدود. تزداد القيمة مع أداء الرياضي.',
    nft_filters: 'مرشحات متقدمة',
    nft_showing: 'عرض',
    nft_of: 'من',
    nft_drops: 'قطرات NFT',
    nft_no_drops: 'لا توجد قطرات تتطابق مع مرشحاتك',
    nft_rarity: 'الندرة',
    nft_price_range: 'نطاق السعر',
    nft_availability: 'التوفرية',
    nft_card_number: 'البطاقة #',
    nft_price: 'السعر',
    nft_available: 'متاح',
    nft_sold_out: 'نفذت الكمية',
    nft_drop: 'قطرة',
    nft_mint_now: 'صك الآن',
    nft_minting: 'جارٍ الصك...',
    nft_owned: '✓ مملوك',
    nft_upcoming: 'قادم',
    nft_my_collection: 'مجموعتي',

    // Secondary Market
    secondary_subtitle: 'التداول من نظير إلى نظير',
    secondary_title: 'السوق الثانوية',
    secondary_description: 'اشترِ وبع رموز الرياضيين و NFTs مباشرة من المجمعين الآخرين. 5% رسم منصة على جميع التداولات.',
    secondary_list_for_sale: 'قائمة للبيع',
    secondary_no_listings: 'لا توجد قوائم متاحة',
    secondary_quantity: 'الكمية',
    secondary_per_unit: 'لكل وحدة',
    secondary_total: 'الإجمالي',
    secondary_buy_now: 'اشترِ الآن',
    secondary_processing: 'جارٍ المعالجة...',
    secondary_create_listing: 'إنشاء قائمة',
    secondary_select_asset: 'اختيار أصل للبيع',
    secondary_listing_price: 'سعر القائمة (EUR)',
    secondary_enter_price: 'أدخل السعر لكل وحدة',
    secondary_sale_price: 'سعر البيع',
    secondary_platform_fee: 'رسم المنصة (5%)',
    secondary_you_receive: 'تستقبل',
    secondary_cancel: 'إلغاء',
    secondary_create: 'إنشاء قائمة',
    secondary_creating: 'جارٍ الإنشاء...',

    // Secondary Market
    secondary_subtitle: 'التداول من نظير إلى نظير',
    secondary_title: 'السوق الثانوية',
    secondary_description: 'اشترِ وبع رموز الرياضيين و NFTs مباشرة من المجمعين الآخرين. 5% رسم منصة على جميع التداولات.',
    secondary_list_for_sale: 'قائمة للبيع',
    secondary_no_listings: 'لا توجد قوائم متاحة',
    secondary_quantity: 'الكمية',
    secondary_per_unit: 'لكل وحدة',
    secondary_total: 'الإجمالي',
    secondary_processing: 'جارٍ المعالجة...',
    secondary_buy_now: 'اشترِ الآن',
    secondary_create_listing: 'إنشاء قائمة',
    secondary_select_asset: 'اختيار أصل للبيع',
    secondary_listing_price: 'سعر القائمة (EUR)',
    secondary_enter_price: 'أدخل السعر لكل وحدة',
    secondary_sale_price: 'سعر البيع',
    secondary_platform_fee: 'رسم المنصة (5%)',
    secondary_you_receive: 'تستقبل',
    secondary_list_success: 'تم إدراجه في السوق!',
    secondary_list_error: 'فشل إنشاء القائمة',
    secondary_purchase_success: 'تم الشراء بنجاح!',
    secondary_purchase_error: 'فشل الشراء',
    secondary_fill_fields: 'يرجى ملء جميع الحقول',
    secondary_cancel: 'إلغاء',
    secondary_creating: 'جارٍ الإنشاء...',
    secondary_create: 'إنشاء قائمة',
    secondary_top_assets: 'الأصول الرئيسية',
    secondary_recent_transactions: 'المعاملات الأخيرة',
    secondary_activity_empty: 'لا يوجد نشاط بعد',
    secondary_transactions_empty: 'لا توجد معاملات بعد',

    // Gamification
    gamification_subtitle: 'مركز المشاركة',

    // Global Leaderboard
    leaderboard_subtitle: 'نظام الترتيب العالمي',
    leaderboard_title: 'لوحة الأبطال',
    leaderboard_showing: 'عرض',
    leaderboard_empty: 'لا يوجد رياضيون على لوحة الرتب حتى الآن',
    leaderboard_loading: 'جارٍ تحميل الترتيب...',

    my_subscriptions: 'اشتراكاتي',
    analytics_admin: 'تحليلات الإدارة',
    analytics_personal: 'تحليلاتي الشخصية',
    analytics_title: 'مقاييس الأداء',
    analytics_admin_desc: 'راقب المشاركة على المنصة، واتجاهات التصويت وأداء الرهانات',
    analytics_personal_desc: 'تتبع مشاركتك، ودقة التصويت وأداء الرهانات عبر الزمن',
    analytics_engagement: 'المشاركة',
    analytics_voting: 'التصويت',
    analytics_betting: 'الرهانات',
    voting_hub_title: 'مركز التصويت',
    voting_no_live: 'لا توجد فعاليات مباشرة',
    voting_view: 'عرض الأصوات →',
    voting_back: '← العودة للفعاليات',
    voting_campaigns: 'حملات التصويت',
    voting_leaderboard: 'الترتيب',
    voting_manage: 'إدارة الحملات',
    voting_no_campaigns: 'لا توجد حملات تصويت نشطة',
    nft_dashboard_portfolio: '// المحفظة //',
    nft_dashboard_title: 'مجموعة NFT',
    nft_dashboard_total_value: 'القيمة الإجمالية',
    nft_dashboard_nfts: 'NFT',
    nft_dashboard_invested: 'إجمالي الاستثمار',
    nft_dashboard_purchase: 'سعر الشراء',
    nft_dashboard_profit: 'ربح / خسارة',
    nft_dashboard_transactions: 'المعاملات',
    nft_dashboard_buy_sell: 'شراء وبيع',
    nft_dashboard_my_nfts: 'NFT الخاصة بي',
    nft_dashboard_sort_value: 'ترتيب حسب القيمة',
    nft_dashboard_sort_profit: 'ترتيب حسب الربح',
    nft_dashboard_sort_recent: 'ترتيب حسب الأحدث',
    nft_dashboard_empty: 'لا تمتلك أي NFT بعد',
    nft_dashboard_purchase_label: 'الشراء',
    nft_dashboard_current: 'الحالي',
    nft_dashboard_pl: 'ر/خ',
    nft_dashboard_minted: 'تاريخ السك',
    nft_dashboard_return: 'العائد',
    nft_dashboard_last_trade: 'آخر تداول',
    nft_dashboard_history: 'سجل المعاملات',
    nft_dashboard_card: 'البطاقة',
    nft_dashboard_type: 'النوع',
    nft_dashboard_price: 'السعر',
    nft_dashboard_date: 'التاريخ',
    athlete_edit_profile: 'تعديل الملف الشخصي',
    athlete_events_stat: 'الفعاليات',
    athlete_wins_stat: 'الانتصارات',
    athlete_podiums_stat: 'المنصات',
    athlete_rating_stat: 'التقييم',
    athlete_universal_score: 'النقاط الشاملة',
    athlete_badges: 'الشارات',
    athlete_royalty: 'عائدات الإتاوات',
    athlete_event_history: 'سجل الفعاليات',
    athlete_no_events: 'لا توجد فعاليات بعد',
    athlete_achievements: 'الإنجازات',
    athlete_not_found: 'الرياضي غير موجود',
    athlete_invalid_link: 'رابط ملف شخصي غير صالح',
    enhanced_loading: 'جارٍ تحميل الملف الشخصي...',
    enhanced_ugc_label: 'محتويات UGC',
    enhanced_engagement_label: 'إجمالي التفاعل',
    enhanced_points_label: 'النقاط المكتسبة',
    enhanced_tokens_label: 'الرموز المملوكة',
    enhanced_tab_overview: 'نظرة عامة',
    enhanced_tab_ugc: 'ugc',
    enhanced_tab_badges: 'شارات',
    enhanced_tab_events: 'فعاليات',
    enhanced_tab_betting: 'رهانات',
    enhanced_ugc_title: 'سجل محتويات UGC',
    enhanced_no_ugc: 'لا توجد محتويات UGC بعد',
    enhanced_badges_title: 'الإنجازات والشارات',
    enhanced_events_title: 'الفعاليات المحفوظة',
    enhanced_no_events: 'لا توجد فعاليات محفوظة',
    enhanced_creator_level: 'مستوى المنشئ',
    enhanced_engagement_score: 'نقاط التفاعل',
    enhanced_rewards_unlocked: 'المكافآت المفتوحة',
  }
};

export const useTranslation = (lang = 'en') => {
  return (key) => translations[lang]?.[key] || translations.en[key] || key;
};