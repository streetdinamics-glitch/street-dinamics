...
const links = [
  { icon: '📸', label: 'Instagram', url: 'https://instagram.com/streetdinamics' },
  { icon: '🎵', label: 'TikTok', url: 'https://tiktok.com/@streetdinamics' },
  { icon: '▶️', label: 'YouTube', url: 'https://youtube.com/@streetdinamics' },
  { icon: '🟢', label: 'Kick — Live Streams', url: 'https://kick.com/streetdinamics' },
  { icon: '👻', label: 'Snapchat', url: 'https://snapchat.com/add/streetdinamics' },
];

export default function SocialLinksModal({ onClose }) {
  return (
    <div className="fixed inset-0 z-[600] bg-black/95 backdrop-blur-xl flex items-center justify-center p-5">
      <div className="relative bg-gradient-to-br from-[rgba(10,4,18,0.99)] to-[rgba(4,2,8,1)] border border-fire-3/20 clip-cyber p-8 md:p-10 max-w-[460px] w-full text-center">
        ...
        <button onClick={onClose} className="absolute top-3 right-4 font-mono text-[10px] tracking-[2px] text-fire-3/30 hover:text-fire-3 cursor-pointer bg-transparent border-none">✕ CLOSE</button>

        <div className="font-orbitron font-black text-xl tracking-[3px] text-fire-gradient mb-0.5">STREET DINAMICS</div>
        <div className="font-mono text-[9px] tracking-[4px] uppercase text-fire-3/30 mb-6">Follow Us Everywhere</div>
