...
export default function DashboardAdmin() {
  ...
  // Admin tool labels — these are admin-only, keeping IT/EN bilingual labels here is fine
  // but we use translation keys where available
  const adminTools = [
    { to: '/CreateEvent', emoji: '➕', label: t('admin_create_event'), desc: 'Nuovo torneo SD', color: 'green' },
    { to: '/Admin', emoji: '🔧', label: t('admin_panel'), desc: 'Tutte le funzioni admin', color: 'green' },
    { to: '/Analytics', emoji: '📊', label: t('nav_analytics'), desc: 'Statistiche piattaforma', color: 'fire' },
    { to: '/NFTDashboard', emoji: '💎', label: 'NFT & Card', desc: 'Gestione drop e token', color: 'purple' },
    { to: '/VotingHub', emoji: '🗳️', label: t('voting_hub_title'), desc: 'Gestisci campagne di voto', color: 'yellow' },
    { to: '/Web3', emoji: '⛓️', label: t('nav_web3'), desc: 'Configurazione blockchain', color: 'purple' },
  ];
    ...
    <div className="relative min-h-screen bg-cyber-void text-[var(--text-main)]">
      ...
      <div className="pt-[80px] section-container max-w-5xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <StatCard emoji="🔴" label="Live" value={liveEvents} color="red" />
          <StatCard emoji="📅" label={t('events_title')} value={upcomingEvents} color="fire" />
          <StatCard emoji="⏳" label={t('dash_stat_regs')} value={pendingRegs} color="yellow" />
          <StatCard emoji="🤝" label="Sponsor" value={activeSponsors} color="green" />
        </div>
            ...
            {pendingRegs > 0 && (
              <div className="flex items-center justify-between p-3 border border-yellow-500/40 bg-yellow-500/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-orbitron text-sm text-yellow-400">{pendingRegs} {t('dash_stat_regs')} pending</span>
                </div>
        ...
        <div className="mb-8">
          <p className="font-mono text-[10px] tracking-[5px] uppercase text-green-500/40 mb-4">{t('dash_recent_events')}</p>
          {events.length === 0 ? (
            <div className="border border-white/5 p-4 text-center">
              <p className="font-rajdhani text-white/30">{t('dash_no_events')} <Link to="/CreateEvent" className="text-green-400">{t('dash_create_first')}</Link></p>
            </div>
          ) : (
            <div className="space-y-2">
              {events.slice(0, 5).map(e => (
                <div key={e.id} className="flex items-center justify-between p-3 border border-white/5 bg-white/2">
                  ...
                  <span className={`font-mono text-[10px] px-2 py-1 border ${e.status === 'live' ? 'border-red-500/60 text-red-400 animate-pulse' : e.status === 'upcoming' ? 'border-yellow-500/40 text-yellow-400' : 'border-white/20 text-white/30'}`}>{e.status?.toUpperCase()}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="mb-8">
          <p className="font-mono text-[10px] tracking-[5px] uppercase text-green-500/40 mb-4">{t('dash_admin_tools')}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {adminTools.map(l => <AdminLink key={l.to} {...l} />)}
          </div>
        </div>
      </div>
      ...
      <div className="section-container max-w-5xl">
        {ugcPending.length > 0 && (
          <div className="mb-2 p-3 border border-purple-500/40 bg-purple-500/10 flex items-center justify-between">
            <span className="font-orbitron text-sm text-purple-400">🎴 {ugcPending.length} UGC pending</span>
          </div>
        )}
        <UGCReviewPanel />
      </div>
